...
Each resource connection is a compiled logistics pipeline with three explicit stages: loader, line, and unloader, plus an ordered list of occupied region cells. The first/last cells bind to exterior port cells and every step is cardinal. Device footprints, deposits, bounds, and self-intersections are compile-time obstacles. Each stage references a transport-capable Device asset whose declared roles determine where it may be used. Its `planTransport()` hook computes stage capacity and duration. The compiler sums nominal latency, derives independent loader/line/unloader clocks, and assigns one travel clock plus one downstream direction to every physical belt cell.

Local cargo is not represented by an end-to-end delay. Each item is a stateful entity in `loading`, `belt`, or `unloading`; while on a belt it owns exactly one concrete cell. Movement frees its old cell and claims the next one atomically. An occupied cell or busy unloader holds the item in place, and that pressure propagates upstream through the occupied line. Connections can merge by sharing a same-direction suffix, with deterministic round-robin arbitration at contested cells. They cannot diverge from a shared cell without an explicit junction. Build cost and area count every physical cell once. Semantic position, blocked/unblocked, unload-start, and arrival events expose the same state to replay and Studio.
...
The old `behavior.kind` execution switch does not exist. Processes are not shared assets or an engine-global Recipe database: each project owns `processes/*.process.json`, and their hashes participate in run identity. A Device may declare supported Process categories, an exact rational speed multiplier, configurable input/output buffer sets, semantic capabilities, and any number of named buffers and ports. Each blueprint Device instance selects a recipe and maps every Process Resource to a permitted buffer; compilation narrows that instance's port contracts to those bindings. Its `DeviceProgram` owns the final local throughput decision and returns one of five declarative decisions:
...
Before simulation, `planProductionCapacity()` treats `Objective.targetRatePerMinute` as an executable industrial specification. It recursively expands the selected recipe chain, aggregates converging Resource demand, and compares required cycles with configured capacity. The same plan sizes extractors, accounts for generator fuel, tests finite reserves over Scenario time, checks Process input/output connection envelopes, derives station fleet demand from route trip capacity, and builds a regional power envelope. Every gap retains a stable kind, entity, and explanation. The plan is returned by `inm plan`, rendered in Studio, and injected into every research proposal; after a KEEP it is recomputed from the new compiled blueprint.

`synthesizeFactoryBlueprint()` starts from the same Objective but constructs a blueprint instead of auditing one. It recursively selects the fastest compatible project-local Process/Device pair for every required intermediate, keeps exact multi-Resource buffer bindings, sizes machine and extractor counts, assigns upstream work to regions with the required finite deposits, and keeps the final Process near the target boundary. Graph fan-in/fan-out becomes explicit junction Devices; missing local producers become finite-fleet station networks between regions. Device placement and port assignment are deterministic, and local connections are routed from raw inputs toward finished goods so upstream lanes reserve the tight machine approaches before downstream routes. Finally, renewable distributors are sized against the complete regional Device and transport endpoint load. The generated blueprint crosses the ordinary compiler, capacity planner, simulator, Objective constraints, and fixture system—there is no separate permissive execution path for generated factories.
