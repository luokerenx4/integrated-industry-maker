...
Longer-range logistics is declarative. A station-capable Device exposes a project-local internal backing buffer, supported network kinds, a fixed number of Resource slots, and a carrier-energy buffer/maximum charging envelope. Each station instance explicitly configures its grid charging request and its own compatible carrier asset/count inside a network. A blueprint groups station instances into `local` or `inter-zone` networks and marks each station/Resource slot as `supply`, `demand`, or `storage` with an independent capacity. Slot contracts are combined per station across networks, narrow the compiled buffer, and must fit both the asset slot count and backing-buffer total capacity. Inventory policy may retain a supply reserve for local use, cap remote replenishment at a demand target, and assign competing-route demand/supply priorities. The network then applies FIFO, round-robin, or shortage-first within equal explicit priorities. Shortage-first recursively traces a demand station's outgoing same-Resource, same-buffer pass-through graph to exact Process/fuel/Objective/terminal-buffer contracts, deduplicates converged leaves, sums their batch coverage, counts resident plus local and remote inbound cargo, and uses production critical depth before deterministic cursor ties. Local routes remain in one industrial zone; inter-zone routes connect different zones. Zone coordinates plus device-local coordinates produce deterministic integer route distance, which represents line-haul travel, site boundaries, loading, and dispatch difficulty. It is passed into the carrier's TypeScript `planTransport()` hook and carrier mission-energy formula. At runtime each source station owns reusable capacity: a route spends its complete mission energy, reserves one home-fleet member and destination Resource quota, removes only inventory above its supply reserve, and emits departure, cargo-arrival, and carrier-return events. Cargo deposits after the loaded outbound leg, while the carrier remains unavailable through the equally timed empty return. Local belts reserve the same physical quotas but are not stopped by remote policy thresholds. Failures, regional power, and source energy gate new dispatches; cargo in flight and returning carriers remain explicit state.
...
Evidence authority is narrower and stricter in the useful direction. Core computes one `executionHash` from the exact selected closure: reachable Resource/Process/Route semantics, placed/transport/fleet Device physics and runtime source, effective plans and contracts, selected World/Blueprint/Production Plan/Scenario/Objective semantics, compiled topology, and power grids. Presentation and unselected qualifications/options remain outside it. Adding an unused project-local alternative does not stale unrelated evidence; selecting it or changing any reachable semantic/runtime input does.
...
The injection interface is uniform even though each device's implementation and configuration are private. For a Process-bound Device, the compiler injects a resolved, buffer-bound Process plan containing the selected mode, exact job duration, exact active power, and aggregated input/output batches. Programs see that plan and local buffers, not mutable factory state. A `start` decision must reproduce that plan exactly. The host validates actions and remains the only authority allowed to write buffers, schedule events, allocate power, or update metrics. Transport-capable programs additionally implement `planTransport()`. Its stage is one of `loader`, `line`, `unloader`, or `carrier`; carrier assets also declare whether they support local, inter-zone, or both network kinds.

Setup-sensitive production stays on the host side of that boundary. Processes declare retained setup groups, Device assets declare a directed changeover matrix with transition-specific time and power, and Scenarios declare tick-zero setup state. Before calling the Device program for ready work in another group, the host schedules the exact non-productive powered transition. Blueprint dispatch can minimize or deliberately spend those changes, but cannot rewrite their physical cost. See [[docs/design/equipment-changeover]].

Route-aware operation dispatch is also host-owned. A Blueprint may rank ready operations by the least slack of their resident tracked lots. The simulator derives nominal remaining work from the immutable Route and Process catalogs, uses the selected operation's exact compiled duration, and writes the decisive lot, remaining ticks, and slack into `device.start`. It does not predict future queues or rewrite due dates. See [[docs/design/work-center-dispatch]].

Batch formation also stays host-owned. Fixed Processes declare their own exact lot counts, durations, material, and power. A Blueprint may qualify a multi-lot Process plus a compatible smaller alternative and prefer the full batch for a bounded time. The simulator schedules the exact timeout, preserves held lot identities, and enters explicit tail-drain state after release; it never invents a partial Process job. See [[docs/design/batch-processing]].

Tracked-lot termination stays on the host side. A terminating Process may consume one tracked work lot and produce only untracked products; the host ends the exact Route identity after successful physical work while downstream product inventory remains fungible. An Objective may separately name that source family for work-order service metrics and freeze several fungible-product delivery contracts. Contract demand floors, unit value, shortfall penalty, and optional fulfillment gates stay evaluator-owned; Blueprint `contract-value` dispatch can optimize shared equipment against marginal portfolio value without editing those terms. Scheduled untracked material deliveries enter through real receiving buffers and ordinary transport. See [[docs/design/industrial-boundaries]] and [[docs/design/delivery-contracts]].
...
Each result is keyed by engine version, selection-scoped execution identity, named World/Blueprint/Production Plan/Scenario/Objective hashes, seed, duration, and event limit. Full catalog-inventory hashes are intentionally excluded. `resultHash` covers the run key, ordered event stream, final state, and metrics.

## Evaluation

Evaluation exposes quantities, treated lots and agent consumption, contract demand/delivery/valued/above-demand/attainment/value/shortfall, throughput, completed orders, on-time delivery, inspection/rework/scrap/escape outcomes, good and first-pass yield, consumed/fuel/stored/charged/discharged energy, build cost, occupied area, per-machine utilization, idle/waiting/blocked/unpowered time, Objective-scoped WIP plus total/excluded/per-Resource inventory accounting, belt occupancy/blocking, loader/unloader utilization, per-connection flow, transport-only energy, congestion, bottleneck, infeasibility, score breakdown, and final score.

Before simulation, `planProductionCapacity()` treats `Objective.targetRatePerMinute` as an executable industrial specification and `Objective.targetRegion` as its delivery boundary. Runtime throughput counts target-Resource consumption only in that region. A deterministic two-phase simplex solves all configured Process types as one non-negative material-balance system. Phase one minimizes weighted finite raw-resource extraction; phase two preserves that optimum while minimizing continuous installed Process capacity. This supports converging demand, alternative recipes, multiple outputs, direct and indirect recycle loops, and explicit surplus without pretending the graph is a tree. Blueprint synthesis adds a second formulation whose constraints are keyed by `(Resource, region)`: Process variables are region-qualified, raw-source variables have Scenario-derived reserve caps, and directed transport variables subtract from one regional balance and add to another with world-distance cost. Target-producing Processes are constrained to `targetRegion`, making upstream placement and intermediate shipment the optimized degrees of freedom. The resulting cycle rates size Process Devices and extractors, account for generator fuel, credit Production-Plan-scheduled tracked lots and purchased materials as external supply, test finite reserves over Scenario time, check every Process input/output connection envelope, derive station fleet demand from route trip capacity, and build a regional power envelope. For shared equipment, a deterministic bipartite maximum-flow allocates required device-time across exact Process/mode qualifications and finite physical Device clocks; one work center can no longer satisfy several operations at full rate simultaneously. Every gap retains a stable kind, entity, and explanation. The plan is returned by `inm plan`, rendered in Studio, and injected into every research proposal; after a KEEP it is recomputed from the new compiled blueprint. See [[docs/design/fab-capacity-planning]].
...
The built-in research agent turns Objective target-rate gaps, structural static semantics, diagnostic codes, and measured runtime flow into bounded strategies: switch a Device to a higher-capacity compatible recipe with a complete generated binding object, duplicate a real Device that relieves an overloaded qualified toolset, add the next planned dedicated Process Device toward the required count, replace a bottleneck local-logistics stage when either its physical contract is insufficient or its recorded connection flow is saturated, expand an undersized station fleet, extend power to a disconnected consumer, add measured generation/storage for observed energy loss, or duplicate a measured high-utilization processor. Planned capacity strategy keys include the Process/toolset and configured-count transition, so multi-step expansion can continue without repeating an identical experiment. Configured operation and simultaneous-rated power envelopes are descriptive topology views; their signed differences cannot create research proposals. A logistics replacement selects only faster compatible project-local Device assets and reports the measured Resource mix that justified the edit. `ResearchInput.history` records strategy keys, KEEP/REVERT decisions, and score deltas from earlier iterations in the same invocation so a rejected experiment is not immediately repeated. External agents receive the same plan, diagnostics, metrics, and history contract.

Resource assets, Device assets and their scripts, Production Plans, Scenarios, Objectives, region definitions, simulator code, evaluator code, and score definition cannot be patched. A proposal is applied to a copy, schema-validated, compiled, simulated, and evaluated before the score comparison.
