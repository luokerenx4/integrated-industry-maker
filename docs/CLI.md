...
Human output starts with the operation id, then reports live continuation provenance, baseline cache reuse, elapsed case timing, seed when applicable, selected branch, proposal-time promotion boundary, observed loss chain, addressed loss or repair case, explicit historical driver replay when required, node-local exhaustion and next node, decision basis, current-best aggregate delta, and either the limiting case or first guardrail violation with its allowed regression and leading Objective-component drivers on stderr. `--progress off` disables display while the final envelope still retains execution counts and timing. `--progress ndjson` emits one compact versioned `{ type: "progress", command: "design", execution, progress }` envelope per Core event on stderr, suitable for an Agent reading incrementally while `--json` reserves stdout for the single final success envelope. Each V3 Core progress value has a deterministic sequence, exact Program/Benchmark, direct source/reused-iteration identity or `null`, cumulative maximum, previous evaluated count, additional Candidate budget, completed/planned case-evaluation counts, and phase-specific timing/cache, driver evidence, proposal boundary/target, replay, exhaustion, complete current-best decision evidence, or result identity. Progress is operational and does not participate in the immutable result hash.
...
Manages the local Studio workbench. `start` creates a background service or idempotently reuses the exact healthy, source-current root already on that port. When the same root is running stale source, `start` replaces it automatically only if ignored project-local lifecycle state verifies its PID and running source hash; an unverifiable process is reported and never killed. `status` reports its project, URL, PID, ignored project-local log, and `current`, `stale`, or `not-running` source state; `restart` replaces only that verified service with current source; and `stop` releases only that verified service. These four operations support `--json`. `serve` is the explicit foreground primitive for tests and direct debugging. Start, restart, and serve accept `--no-open`. The default port is `4176`.

Studio exposes `GET /api/health` with its protocol identity, engine version, PID, resolved input root, optional workspace project selection, deterministic runtime-source hash, start time, and URL. Managed JSON lifecycle output exposes both the expected hash from the calling checkout and the running hash. A port occupied by another INM root or by an unknown service is an error, and INM never kills the occupant merely because a port is requested. Managed state and logs live below the selected root's ignored `.inm/studio/` directory. See [[docs/design/development-operations]].
