...
Treats the Objective's required target rate as an industrial specification. The planner recursively expands active recipes and reports required cycles/min, configured versus required machine counts, raw Process and generator-fuel demand, extractor capacity, finite-reserve lifetime over the Scenario, input/output connection envelopes, shared station carrier counts, and regional generation headroom. Its explicit gap list is deterministic JSON input for research agents as well as a human CLI review surface.
