...
Human output contains per-case baseline/candidate scores and capacity state followed by stable `baseline_score`, `benchmark_score`, `score_delta`, `patch_operations`, `semantic_changes`, and `verdict` lines suitable for a Coding Agent loop. `--json` returns the exact patch, semantic changes, every case result and gate reason. Normal evaluation is read-only and writes no run artifact. A valid `DISCARD` or `UNCHANGED` experiment still exits successfully so an Agent can record it; invalid files, lock drift, or failed simulation return a non-zero error.
