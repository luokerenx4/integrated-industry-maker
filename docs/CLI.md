...
Clicking a Device opens a scoped inspector for its runtime status, utilization, recipe/mode batches, buffer contracts, extraction/generation plan, build and power data, grid membership, diagnostics, and incoming/outgoing links. Clicking any real belt cell opens its physical connection inspector with loader/line/unloader contracts, endpoint power, stack and timing limits, current replay cargo, measured throughput/resource mix, utilization, in-flight inventory, and blockage; endpoint and link buttons navigate between the two inspector kinds. Selection is read-only and project-scoped. Studio is also read-only with respect to run history: opening or inspecting a project never writes a baseline artifact; `inm simulate` or `inm research` must create runs explicitly.
