...
Lists project-local Design Programs when `--program` is omitted. Selecting a program distinguishes an authored seed from a project-synthesized input, reports synthesis provenance and normalized seed hash, separately names the Benchmark candidate Blueprint/hash it would eventually update, and returns the locked Benchmark, driver case, exact current-best case guardrail, hashes, allowed decision families, bounded budget, target-rate capacity state, flow diagnostics, declarative/opaque Device counts, and topology without creating simulation or review evidence.

`--run` explicitly executes bounded design search. Before each proposal, the driver case supplies deterministic metrics, an exact metrics hash, and a Core-derived tracked-fab loss chain. Project proposal-provider API V3 must name one observed bucket its candidate addresses and receives the Core-validated target of every prior KEEP or REJECT attempt, allowing deterministic target diversification within the bounded budget. The claim is recorded but every in-memory candidate is still accepted only through the complete locked multi-case Benchmark, positive aggregate current-best improvement, and the Program's current-best case guardrail. `--max-candidates` may lower the manifest budget but cannot raise it. Execution never edits the seed or candidate Blueprint and writes or reuses only:
...
JSON sections are `summary`, `static`, `iterations`, `best`, `runs`, and `all`. Every valid iteration exposes its driver metrics hash, complete source-neutral loss profile, addressed loss, exact strategy family, hypothesis, proposal and Blueprint hashes, restricted patch, fixed-baseline robust evaluation, and `decisionEvidence`: one basis, current-best aggregate comparison, ordered per-case comparisons with resolved maximum regression and pass result, guardrail kind/result/violations, limiting-case id, and evaluator gate reasons when applicable. Invalid candidates expose their validation error instead. Human run and reopen output preserve the compact observed loss → target → decision reason relationship. Returned `design-run` and promoted `candidate` artifacts are immutable. See [[docs/design/design-programs]].
