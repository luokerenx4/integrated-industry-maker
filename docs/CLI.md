...
When a completed tracked-lot run exactly matches every current project hash, inspection ranks the measured fab loss chain ahead of generic nominal warnings while leaving capacity blockers first. `--section losses --json` returns delivery-portfolio, release/admission, tracked-lot queue congestion, active productive-equipment input starvation, batching, setup, maintenance/qualification, tooling, facility, failure, power, transport, Q-time, and verified yield/quality buckets with exact run identity and subjects. Delivery evidence retains contract shortfall, overflow, and value separately. Input starvation uses observed between-job opportunity gaps, exposes ranked Device/process contributors, and keeps warm-up, completed-campaign drain, separately measured unavailability, and normally sparse exception tools out of the ranked signal. Verified yield contributors retain exact origin mechanism, Process, Device, Route step, defect classes, lot ids, and downstream rework/repair/persistence/scrap/escape counts; unfinished WIP is not counted as failed inspection yield. These are overlapping prioritization signals, not additive lost-output claims. See [[docs/design/fab-loss-attribution]].
