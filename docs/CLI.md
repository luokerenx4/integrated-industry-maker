...
When a completed tracked-lot run exactly matches every current project hash, inspection ranks the measured fab loss chain ahead of generic nominal warnings while leaving capacity blockers first. `--section losses --json` returns delivery-portfolio, release/admission, productive queue/starvation, batching, setup, maintenance/qualification, tooling, facility, failure, power, transport, Q-time, and yield/quality buckets with exact run identity and subjects. Delivery evidence retains contract shortfall, overflow, and value separately; passive customer/Scrap Sinks are not treated as starved industrial equipment. These are overlapping prioritization signals, not additive lost-output claims. See [[docs/design/fab-loss-attribution]].
