...
The brief identifies the leading active Workbench diagnostic and returns stable run-qualified Studio routes for the complete Factory replay, up to three relevant Device/Connection or Catalog subjects, and the matching Analysis evidence. When every realized loss is bounded and no structural warning remains, the diagnostic is `null` and only the run-qualified overview is required; a new intervention then needs fresh causal evidence. The brief also returns the statements a human or reasoning Agent must make before authoring an intervention: visible behavior, relationship to structured evidence when present, a falsifiable hypothesis, the smallest exact change, and expected quantitative/visual guards.
...
`start` creates a background service or idempotently reuses the exact healthy, source-current target. When that target is running stale source, `start` replaces it automatically only if ignored project-local lifecycle state verifies its live ownership; an unverifiable process is reported and never killed. `status` reports project, URL, selected port and provenance, PID, ignored project-local log, and `current`, `stale`, or `not-running` source state. `restart` replaces only that verified service and `stop` releases only that verified service. Multiple live instances of one target are reported as an explicit ambiguity. These four operations support `--json`; start and restart accept `--no-open`.

`serve` is the explicit foreground primitive for tests and direct debugging, accepts `--no-open`, and defaults to `4176`. Studio exposes `GET /api/health` with its protocol identity, engine version, PID, resolved input root, optional workspace project selection, deterministic runtime-source hash, start time, and URL. Managed JSON lifecycle output exposes both the expected hash from the calling checkout and the running hash. A port occupied by another INM root or by an unknown service is never killed merely because it was discovered or requested. Managed state and logs live below the selected root's ignored `.inm/studio/` directory. See [[docs/design/development-operations]].
