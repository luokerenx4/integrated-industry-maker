...
Builds the shared [[docs/design/operator-workbench]] snapshot for the effective World, Blueprint, Scenario, and Objective. Human output is a compact orientation view: exact input hashes, normalized delivery contracts, Objective WIP scope and compatible-run inventory evidence, separate capacity/flow/evidence/review status, the one shared next action, topology/catalog/evidence counts, prioritized diagnostics, and available/conditional operations with their effects. `--json` defaults to a bounded orientation summary. Sections expose `next-action`, `diagnostics`, `losses`, `dispositions`, `catalog`, `runs`, `experiments`, `candidates`, and `operations`; `--section next-action --json` and the envelope's sole `nextActions` item are the exact Core action, while `--section all --json` returns the complete V8 `ProjectWorkbenchSnapshot`. Inspection is read-only and an invalid explicit selection never falls back to a project default.

When a completed tracked-lot run exactly matches every current project hash, inspection ranks the measured fab loss chain ahead of generic nominal warnings while leaving capacity blockers first. `--section losses --json` returns delivery-portfolio, release/admission, tracked-lot queue congestion, active productive-equipment input starvation, batching, setup, maintenance/qualification, tooling, facility, failure, power, transport blocking, Q-time, and verified yield/quality buckets with exact run identity and subjects. Delivery evidence retains contract shortfall, overflow, and value separately. V7 input starvation intersects observed runtime material-shortage intervals with between-job opportunities, exposes ranked Device/Process contributors plus exact Resource, Buffer, required/resident quantity, immediate connection/source, endpoint status, in-flight quantity, and observed supply state, and leaves any unexplained eligible gap explicitly unattributed. Warm-up, completed-campaign drain, separately measured unavailability, and normally sparse exception tools remain outside the ranked signal. Necessary lot transit remains mean cycle-time context; only positive blocked item-time produces `transport-blocking`, with ordered connection/Resource/flow/capacity contributors and an exact line-contention/endpoint-capacity/endpoint-power/endpoint-failure partition. Human `inspect` prints the same material-shortage states, dominant transport mechanism, and cause totals that machine JSON and Studio receive; ordinary in-flight material, endpoint service, or power evidence is never relabeled as a faster-line opportunity. Verified yield contributors retain exact origin mechanism, Process, Device, Route step, defect classes, lot ids, and downstream rework/repair/persistence/scrap/escape counts; unfinished WIP is not counted as failed inspection yield. Workbench V8 may put one exact still-visible contributor under hash-current bounded deferral after exhaustive rejected Design evidence; `--section dispositions --json` exposes its authority, counts, decision bases, and automatic invalidation boundary separately from the physical loss profile. These are overlapping prioritization signals, not additive lost-output claims. See [[docs/design/fab-loss-attribution]].
...
Within an initial run, Core validates and simulates each locked baseline case once, then reuses that invocation-local evidence for the seed and all candidates. A continuation simulates the current locked baseline once, reuses the source's verified seed and Candidate evaluations, and simulates only newly proposed candidates. Candidate simulations, acceptance gates, metrics, patches, and final hashes are not cached. The prepared baseline never survives the process or bypasses the Benchmark lock.
