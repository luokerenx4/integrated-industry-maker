...
A completed design execution is stored under `design-runs/<program-id>/<result-hash>/`. Its `manifest.json` owns the Program hash and copied current-best/frontier policies, seed and promotion identities, every selected parent and candidate node, proposal-time `promotionBoundary`, branch-local driver evidence, distinct `addressedLoss`/`addressedCase` rationale, fixed-baseline evaluation, leader `decisionEvidence`, parent delta, KEEP/BRANCH/REJECT outcome, pruning, exact searchable/exhausted state after each iteration, ordered node-local `exhaustions`, and the final leader/alternatives plus scheduler and node `searchStatus`. An exhaustion precedes a candidate iteration, retains the Pareto node, consumes no Candidate budget, and records its exact next node; only an empty searchable queue produces `frontier-exhausted`. `best.blueprint.json` is always the leader, never a non-promotable alternative. Source-neutral driver evidence never carries a persisted run id. These are generated immutable evidence rather than authored inputs. Reopening verifies both hashes, recomputes every promotion boundary, and interleaves exhaustion evidence with Candidate iterations to replay the complete deterministic frontier graph and scheduler state. Promotion requires an accepted leader that differs from the unchanged promotion base and produces an ordinary Candidate rather than editing the Blueprint.
