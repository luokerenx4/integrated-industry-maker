...
A completed design execution is stored under `design-runs/<program-id>/<result-hash>/`. Its `manifest.json` owns the seed source/synthesis identity, normalized evaluated seed, separate promotion base, each iteration's driver metrics hash and source-neutral fab-loss profile, proposal `addressedLoss`, fixed-baseline multi-case evaluation, current-best `decisionEvidence`, decision, ranking, and deterministic result hash; `best.blueprint.json` is the exact leading in-memory Blueprint. Decision evidence names one basis and records aggregate plus ordered per-case previous-best/candidate/delta values and a limiting-case id. Source-neutral driver evidence never carries a persisted run id. These are generated immutable evidence rather than authored inputs. Reopening verifies both hashes and the complete seed-to-KEEP decision lineage. Promotion requires an accepted best that differs from the unchanged promotion base and produces an ordinary Candidate rather than editing the Blueprint.
