...
Both qualified recipes must compile to identical material work and one unambiguous output Resource. The named physical Connection must originate at that output port, carry only that Resource, and end in a buffer whose Resource capacity is at least `recoverBelowItems`. Before each non-preemptive job, resident destination inventory plus all exact in-flight cargo chooses recovery below the boundary and normal operation otherwise. `cadenceControl` cannot be combined with `recipeDispatch`, `setupCampaign`, or `batchFormation`. Production start/finish events include the chosen mode, and `cadenceControl.devices` metrics expose the authored boundary plus normal/recovery job counts.
