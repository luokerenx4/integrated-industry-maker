...
The filename is `<id>.design.json`. The Benchmark must be locked and `driverCase` must name one of its cases. `seed` is either an authored Blueprint (`{ "kind": "blueprint", "blueprint": "<id>" }`) or an in-memory project synthesis (`{ "kind": "synthesis", "inputBlueprint": "<id>" }`). The Benchmark candidate Blueprint remains the separate promotion target. Optional `focus` is either `{ "kind": "broad" }` or `{ "kind": "losses", "losses": ["<fab-loss-id>", ...] }`; omission means broad. Focus is a Workbench routing declaration, not permission to fabricate or ignore proposal-time loss evidence. A loss-focused missing Program outranks a broad missing Program only when its declared loss matches the active compatible-run diagnostic. Required `currentBestGuardrail` is `{ "kind": "unrestricted" }`, a `uniform` non-negative `maximumCaseScoreRegression`, or a `case-specific` map that names every Benchmark case exactly. Zero uniform regression is strict leader preservation. Required `frontier.maximumAlternativeBranches` is an integer from zero through eight and bounds retained non-promotable Pareto states separately from the leader. Both policies participate in Program identity. Decision families are unique stable ids for Blueprint-owned proposal strategies, not permissions to edit evaluator inputs. `budget.maxCandidates` is a positive integer no greater than 100 and bounds the new Candidate evaluations in each initial or continuation invocation. See [[docs/design/design-programs]].

A completed design execution is stored under `design-runs/<program-id>/<result-hash>/`. Its strict V3 `manifest.json` owns the Program hash and copied current-best/frontier policies, seed and promotion identities, every selected parent and candidate node, proposal-time `promotionBoundary`, branch-local driver evidence, distinct `addressedLoss`/`addressedCase` rationale, fixed-baseline evaluation with required per-case baseline/candidate cadence-control maps, leader `decisionEvidence`, parent delta, KEEP/BRANCH/REJECT outcome, pruning, exact searchable/exhausted state after each iteration, ordered node-local `exhaustions`, and the final leader/alternatives plus scheduler and node `searchStatus`. Each cadence entry includes its authored policy boundary and measured normal/recovery job counts; no control is `devices: {}`. `continuation` is `null` for an initial run; otherwise it records a direct `sourceResultHash`, exact `reusedIterations` and `reusedExhaustions`, and the positive `additionalCandidateBudget`. Manifest `budget.maximum` and `budget.evaluated` are cumulative so Candidate numbers stay stable across the complete copied history. An exhaustion precedes a candidate iteration, retains the Pareto node, consumes no Candidate budget, and records its exact next node; only an empty searchable queue produces `frontier-exhausted`. `best.blueprint.json` is always the leader, never a non-promotable alternative. Source-neutral driver evidence never carries a persisted run id. These are generated immutable evidence rather than authored inputs.
