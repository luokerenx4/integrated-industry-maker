...
The `design` summary keeps seed provenance and search risk policy machine-visible. `program.seed` declares authored Blueprint versus synthesis input; `program.currentBestGuardrail` declares unrestricted, uniform, or exact case-specific leader regression budgets; `program.frontier` bounds non-promotable alternatives; `seed` reports source, synthesis, and normalized evaluated hashes; `promotionBase` separately identifies the Benchmark candidate file and optimistic-concurrency hash. Iteration `evaluation` retains fixed-Benchmark-baseline semantics while `decisionEvidence` names the leader comparison and `frontierEvidence` names the parent, parent delta, KEEP/BRANCH/REJECT outcome, pruning, leader and next selection order. Reopened and newly executed results return the same identities, and a promotion next action appears only for an accepted leader with a non-empty recorded promotion patch.
