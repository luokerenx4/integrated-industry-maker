...
Long-running Design execution has one explicit secondary channel. `inm design --run --progress ndjson --json` writes compact versioned progress envelopes to stderr and exactly one ordinary success envelope to stdout at completion. Each progress record is `{ "schemaVersion": 1, "type": "progress", "command": "design", "progress": ... }`; the nested value is the same Core `DesignRunProgress` projected by Studio. Proposal diagnosis carries the exact driver metrics hash and Core-derived fab-loss profile; proposal completion names the observed bucket the strategy addresses; candidate completion carries the same current-best aggregate/case comparison, resolved per-case regression budgets, guardrail result/violations, decision basis, limiting case, and evaluator-owned gate reasons later stored in the immutable iteration. The stream is ordered and deterministic, contains no timestamps, and reports actual named phases and completed/planned simulation work rather than a wall-clock estimate. `--progress human` formats the same loss chain, target, decision basis, and either limiting or violated case for a terminal, while `--progress off` disables the channel. No other stderr text may be mixed into NDJSON mode.
...
The `design` summary keeps seed provenance and search risk policy machine-visible. `program.seed` declares authored Blueprint versus synthesis input; `program.currentBestGuardrail` declares unrestricted, uniform, or exact case-specific incumbent regression budgets; `seed` reports source, synthesis, and normalized evaluated hashes; `promotionBase` separately identifies the Benchmark candidate file and optimistic-concurrency hash. Iteration `evaluation` retains fixed-Benchmark-baseline semantics while `decisionEvidence` explicitly names the evolving current-best comparison and resolved guardrail outcome, so an Agent never has to infer which delta or policy decided KEEP/REJECT. Reopened and newly executed results return the same identities, and a promotion next action appears only for an accepted result with a non-empty recorded promotion patch.
