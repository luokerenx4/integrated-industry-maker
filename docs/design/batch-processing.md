...
The later commissioned factory exposes a different, event-backed boundary: run `058-simulate` records three anneal Q-time violations caused specifically by batch-companion wait. Its project-local TypeScript sweep compares batch-only, rapid-only, and five bounded fallback windows with physical service-capacity alternatives. The accepted Blueprint still qualifies both immutable Processes, but uses `maximumWaitTicks: 0`: a complete preferred three-lot job may start immediately, while a ready one-lot rapid job is never withheld merely to form a future batch. This is not a fractional batch or altered furnace recipe.

That zero-wait choice also governs readiness evidence before a lot is resident. If the preferred three-lot Process is incomplete, runtime selects the fixed one-lot fallback as the next policy-resolved plan even when the fallback itself is still waiting for input. Material-shortage events therefore report the one lot that would actually resume production, not three lots for a preferred Process that the zero-wait policy will bypass. When three compatible lots are already resident, the preferred Process still wins and executes one complete three-lot job. Positive wait policies retain their explicit hold/release state instead.

Reviewed run `059-simulate` removes all three batch-companion violations and lowers anneal maximum queue from `40.8` to `0.9` seconds. The coupled zero-wait and dual-service Candidate improves every locked case by at least `20.182194`, remains capacity `READY`, and is applied only after an immutable five-case `KEEP` review. This result does not make zero wait a universal default: the earlier interruption result proves the preferred window is operating-point dependent and must remain Blueprint policy judged by the fixed benchmark.
...
Fixed Process batching and local `batch-coherent` dispatch own different boundaries. A Process still decides that one Burn-in job consumes eight devices; the junction policy may only wait for, reserve, and transport exactly that already-authored input count to one qualified rack before rotating. It cannot resize the recipe, declare a partial load complete, or hide sorter/belt work. Conversely, an ordinary round-robin junction remains item-level even when its targets are batch processors. The human or Agent must explicitly choose batch-coherent routing and judge the resulting capacity, WIP, cost, energy, quality, and service evidence.
