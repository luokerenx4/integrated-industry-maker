...
Before keeping an edit, `inm compare` can isolate two named Blueprint files under one World, Production Plan, Scenario, Objective, catalog set, and seed. It returns the exact file patch, industrial semantic changes, both capacity plans, and deterministic score/metric deltas without modifying either Blueprint or writing a run artifact. See [[docs/design/blueprint-comparison]] for the comparison contract.
...
Core executes the strategy twice and rejects nondeterministic output, schema-validates its result, compiles it, requires the same target-rate capacity plan used everywhere else, and exercises the selected Scenario before an atomic write. The strategy is project code, never a shared engine catalog or an evaluator escape hatch: it cannot alter assets, Processes, Routes, Production Plan workload, Scenario disturbances, Objective weights, or Benchmark locks. The generated Blueprint must explicitly own equipment qualification, lanes and sorter endpoints, reusable tooling, facility providers, maintenance, and power like any hand-authored factory.
...
The commissioned input-starvation study applies the same boundary to an equipment control that is cheaper than buying capacity. Project-local TypeScript research rejects three multi-chamber ALD replacements because the live factory has only `50` currency of build-cost headroom. It also separates the exact `42.456 s` furnace-shortage interval into source processing, source input wait, and transport in flight: halving lane time removes `4.950 s` of in-flight evidence but reduces total shortage by only `0.450 s`, because the saved time becomes upstream wait. A physical-looking transport upgrade is therefore not accepted merely because one causal state improves.

The follow-up control grid qualifies `agile-pulse-fast` at `2/3` duration and `3/2` active power, keeps the one-item furnace coverage boundary, and tests `0`, `2`, `5`, and `10 s` persistence thresholds. Five seconds is the sole zero-regression point that also reduces the addressed loss: aggregate current-best score improves `+1.084759`, the limiting facility-interruption case remains `+0.201089`, and mixed-quality furnace shortage falls `42.456 → 40.456 s`. Design Run `206067de7d3566d5793d078f2db05ecbceb3b2ccdd0122ecec70b8b0d5c8a217` selects the exact three-operation policy/recipe patch. Candidate `commissioned-furnace-supply-recovery` and review `04a1b22b3d1d952c98394a838bf054e833c4c8273ac7666da2ced6d398016aac` commission Blueprint `35ef45f0eb537a5e2f7a94b40b1e41bf74fb5f13fb21d067ed996443785ed144`; compatible Run `087-simulate` is the new operating evidence.

Run 114 later tests whether controlling the preceding layer-one etch can improve that same supply path after quality commissioning changes upstream timing. Project-local script `furnace-supply-phase-control.ts` evaluates three explicit etch-to-ALD downstream-coverage thresholds plus the prior always-cycle and input-queue controls under the current five-case Benchmark. The new `5` and `10 s` controls do not change furnace shortage. The `1 s` control recovers only `1.000 s` while adding `2.837567` average WIP equivalent and regressing a current-best case by `2.916394`; the old input-queue control recovers the same second but retains a strict `-0.000099` case regression. The Investigation therefore defers this phase-control family rather than relaxing the uniform-zero commissioning boundary.
