...
`inm benchmark --lock` compiles every baseline case and writes its engine, selection-scoped execution, World, Blueprint, Scenario, and Objective hashes plus a hash of the benchmark contract. Normal evaluation refuses to run when any reachable fixed input or the case contract has drifted. Full unused catalog inventory and candidate Blueprint content are deliberately excluded: a project may author a new option, and the candidate may select it, without falsely changing the unchanged baseline.
