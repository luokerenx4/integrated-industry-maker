...
- one or more weighted cases, each fixing World, Production Plan, Scenario, Objective, seed, and Scenario duration;
...
The memory-fab uses that separation to prevent a real optimizer pathology. Frequent inspection maintenance lowers time-averaged WIP enough to improve every locked case score, but does so with more service work, energy, queueing, cycle time, and late lots. Its current robust contract therefore protects five case-specific `onTimeLots` floors beside six existing delivery, completion, quality, scrap, escape, and release outcomes. The policy remains visible as score-positive economic evidence, while Benchmark and Design reject it with the exact failed due-date counts. Neither the provider nor a UI projection may reinterpret that boundary.

`inm benchmark --lock` compiles every baseline case and writes its engine, selection-scoped execution, World, Blueprint, Production Plan, Scenario, and Objective hashes plus a hash of the benchmark contract. Normal evaluation refuses to run when any reachable fixed input or the case contract has drifted. Full unused catalog inventory and candidate Blueprint content are deliberately excluded: a project may author a new option, and the candidate may select it, without falsely changing the unchanged baseline.

An Agent that should not mutate the candidate directly can instead author `candidates/<id>.candidate.json`. The artifact pins the current candidate hash, names its Benchmark, records the hypothesis, and carries only a restricted Blueprint patch. `inm candidate --review --json` explicitly evaluates it through the locked suite; flagless `inm candidate --json` cheaply reopens recorded evidence without simulation; `--apply` performs one fresh guarded evaluation and writes only a hash-identical reviewed `KEEP`. Studio exposes the same proposal and confirmation boundary to humans. Applied proposals remain as project history: the exact written Blueprint is `verified`, while any later unrelated Blueprint state makes that proposal `stale` rather than silently retargeting it.

Candidate review has two explicit references because they answer different industrial questions. The immutable Benchmark baseline remains the compliance reference and exclusively owns the locked `KEEP`/`DISCARD`/`UNCHANGED` verdict. When the Candidate artifact's hash-pinned current Blueprint is operational, it is separately evaluated under the same cases, seeds, Worlds, Scenarios, Objectives, and capacity planner. `currentFactory.status: evaluated` then reports current-to-proposed aggregate and per-case scores, exact Objective-component deltas, complete metric snapshots, capacity on both sides, and the same hard-outcome values and thresholds. Its `IMPROVED`/`REGRESSED`/`UNCHANGED` label is descriptive evidence, never another application gate or autonomous design decision. A greenfield commissioning shell that only becomes operational after the proposed patch is instead recorded as `not-operational` / `NOT_COMPARABLE` with the exact compile reason; the fixed compliance result remains valid and no historical baseline is relabeled as the current factory. This prevents a large gain over an obsolete greenfield baseline from being mistaken for the incremental effect on a commissioned factory.

A non-KEEP result is retained as a counterexample rather than a dead end. Core deterministically derives a non-authoritative `revisionBrief` from the Candidate, locked result, and current-factory evidence: current-passing outcomes the proposal breaks, negative-score cases, weighted positive Objective components worth preserving, weighted negative components to remove, locked gate reasons, and exact patch paths. The brief says `revise-or-retire` and names `human-or-agent` as decision owner. It does not propose a new policy value, edit the Candidate, or outrank project observation; the next executable step is the exact current-factory observation command.

The pinned base must be a schema-valid Blueprint, but it need not already compile under a Scenario that the patch is intended to commission. Review reads and hashes that base, applies the restricted patch in memory, schema-validates and compiles the proposed Blueprint, then evaluates only that complete proposal through every locked case. Its shared operation context identifies the compiled proposed hash; the separate current/proposed hashes retain optimistic-concurrency evidence. Apply repeats the same order and reports the compiled written Blueprint as its context. This permits a genuine empty-site-to-factory Candidate without relaxing the final compiler, Benchmark lock, KEEP gate, receipt, or post-write hash.

A [[docs/design/design-programs|Design Program]] may also synthesize its working Blueprint from a minimal project input. The Benchmark candidate still names one ordinary, independent mutable file. Fixed-baseline Benchmark gates remain evaluator authority, while the Program separately declares what per-case regression against its evolving current best the bounded search may accept. The immutable Design Run pins both that policy and the file's pre-run hash as its promotion base, and promotion produces the same Candidate artifact described above; synthesis and search never bypass preview, locked re-evaluation, reviewed hashes, or explicit apply.

The Benchmark command is read-only unless `--lock` is explicitly supplied. Candidate review writes only its explicit immutable V2 receipt. Neither operation creates a simulation Run or mutates a Blueprint. Human Benchmark output ends with stable, grep-friendly fields:
...
JSON `cases` and `all` output contains every case score, evaluator-owned baseline/candidate Objective breakdown, exact component delta, required baseline/candidate cadence-control policy and activation counts, capacity state and gap, aggregate score, acceptance reasons, exact ordered industrial-outcome evidence, exact Blueprint patch, and semantic changes. Every metric snapshot carries the complete authored Objective-constraint set, including passing constraints, with stable identity, metric, direction, measured value, threshold, deficit, and pass state. A million-point `constraintPenalty` is therefore a gate signal rather than an opaque diagnosis: Candidate current/proposed evidence can state, for example, that build cost moved from `229,950` to `230,150` against a `230,000` maximum and missed by exactly `150`. An uncontrolled side carries `cadenceControl.devices: {}` rather than omitting the field. Every component sum reconciles with its case score and the delta sum reconciles with `scoreDelta`; an Agent can therefore distinguish a dormant, alternating, or continuously active controller from a WIP or capital penalty without reimplementing Objective weights. Human CLI prints the same per-device and failing-constraint baseline-to-candidate split. The compact summary reports guardrail pass/fail counts, the same per-case hard-outcome values, and bounded current/proposed Objective constraints so an Agent does not need `--section all` merely to understand a gate rejection.
...
- Memory-fab generative target and harness: `examples/memory-fab/blueprints/generated-dram-fab.blueprint.json`, `examples/memory-fab/benchmarks/greenfield-dram-design.benchmark.json`
...
bun run inm candidate examples/memory-fab --candidate stable-furnace-sleep --review
