...
Commitment snapshots discover no topology at runtime. Core compiles, per contract region, the exact Buffers that can hold a contracted Resource, Devices whose Buffers can receive active-job output, local connections that can carry it into the region, and station networks with a matching inbound route. Every contract-value Device evaluation reads delivered counts and the current contents of only those authoritative live sources into one disposable snapshot. The index contains no quantity and requires no invalidation; there is no mutable commitment or reservation ledger beside `FactoryState`.
