...
Continuing that exact frontier with one new Candidate created `d02580bc840c4eca68ba3c83acb77993a35805df4009f021fb73fb316102d500`. Core first recorded candidate 6 as proposal-exhausted, then asked candidate 7 for its next lineage-local proposal. Candidate 8 tested a 10-card CONWIP loop reopening at seven lots. It passed the fixed Benchmark gates but regressed the leader by `-9.960213` overall, including four current-best case violations, so it was rejected as `parent-no-improvement`. Candidate 7 remained leader and candidate 6 remained retained but exhausted. Continuation therefore advanced knowledge without pretending that additional search must improve the factory.

That continuation has now been commissioned. Candidate `commissioned-greenfield-dram-fab` pins the source run and best Blueprint hashes, its 74-operation patch from empty commissioning base `28192fe068a455bcbca606c2b5c2832034b584bdb39ddb5a6cd615fc159021aa`, and proposal hash `a8345e35c82d2894a5b023f319bbc96fd6d8c729a6bbbfbccf00641ed1b5461e`. Its immutable review receipt records a five-case `KEEP` at aggregate `+37.971590`; apply re-evaluated and wrote exact Blueprint `2511191a2ddb542dce3d551ef539e278825a53362576d093cb1ff9381a8c9356`.

The two run hashes above are historical evidence tied to that exact empty promotion base. Applying the leader intentionally moved the target, so those artifacts may still be reopened when their ignored local cache is present, but they can no longer be continued or promoted. Re-running the Program against the commissioned target creates a new identity rather than pretending the old hashes are current. The checked-in Candidate, review receipt, and materialized Blueprint preserve the commissioning chain in a clean checkout even when the Design Run cache is absent.
