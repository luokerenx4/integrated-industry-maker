...
Before candidate 5, the unrepaired parent `candidate-3` has no unused facility repair. Core records its exhaustion and continues `candidate-4` without spending an evaluation. Candidate 5 tests batch formation and is rejected; candidate 6 retains a setup-campaign alternative; that alternative exhausts before candidate 7 and search returns to `candidate-4`. Candidate 7 still evaluates under the original global budget. The run therefore evaluates all `7/7` candidates while honestly retaining `candidate-6` as an exhausted non-dominated alternative.
