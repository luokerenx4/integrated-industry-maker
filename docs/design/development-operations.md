...
`inm session <path>` is the ordinary bridge back into project work. It composes safe source-current lifecycle repair and port discovery, asks that exact Studio for the authoritative shared `ProjectWorkbenchSnapshot`, and opens the unmodified `nextAction.studioRoute`. Human and JSON output identify one strict `project-next-action` target with its Core-owned id, reason, argv, effect, confirmation boundary, typed target, route, and URL. Entry is navigational and never executes the recommended action.

If start observes `recovering`, Session waits only when the live lifecycle still matches the exact input root, optional project, port, expected source hash, supervisor attempted hash, and failure-free recovery phase already verified by managed ownership. The wait is bounded to five seconds and polls the same strict status path. Exact source-current convergence returns as reuse; degradation keeps its detailed failure; timeout, changed source/target, or exiting recovery without current evidence returns `session.studio-recovery-timeout` or `session.studio-recovery-unverified`. Foreign listeners, ambiguous ownership, and persistent degradation never enter this convenience wait.

`inm session <path> --experiment <id>` is the explicit bridge into reconnectable long work. It validates the authored Experiment and opens the direct lightweight route. With `--run`, it starts the same Studio-owned Benchmark operation and returns the operation id and polling URL immediately; it does not hold the terminal open for evaluation. `--run` without an explicit Experiment is rejected before lifecycle discovery or mutation. Human output may open either resolved route, while `--no-open --json` gives an Agent the identical lifecycle and target identity. This is coordination convenience, not a second evaluator: `inm benchmark` remains the independent browser-free synchronous path.
...
- One-command project/Experiment entry: `packages/inm-cli/src/studio-lifecycle.ts`
