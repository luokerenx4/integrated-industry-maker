...
Status: V1 shared evaluation, V2 project-local change-set application, V3 persistent decision loop, V4 immutable Design continuation, V5 commissioned Design provenance, V6 Objective score causality, V7 control activation causality, V8 rejected-Candidate revision handoff, V9 Investigation provenance, V10 continuous factory context, and V11 physical-economics comparison implemented.
...
The workbench uses a native dialog role, named buttons, ordinary buttons for experiment selection, stable test ids for programmatic execution, structured case rows, and textual verdict/diff output. Every locked and current-factory case owns a stable fragment anchor. Above the current-factory cases, one physical-capital ledger shows total build cost, recovered-versus-spent equipment/endpoint/line cost, area and transport-cell changes, plus the exact Objective boundary and remaining headroom. Objective constraints render beneath the corresponding case, open automatically when either side fails, and show current/baseline value, proposed/candidate value, direction, threshold, deficit, and pass transition. Candidate evaluation is not an immutable Factory Run, so these anchors reopen exact retained comparison evidence without inventing a replay identity. The 3D canvas is not required to operate or understand a Benchmark.
