...
Core also owns one operational `BlueprintBenchmarkProgress` V3 stream. Every baseline and Candidate case has ordered start/completion events, completed/total work, explicit `sequential`, `isolated`, or `parallel` execution mode and bounded concurrency, cache reuse, and separate compilation, cache-read, simulation, comparison, Worker startup, and Worker-reuse timings. CLI `auto` retains foreground sequential execution for one/two cases; Studio `background` uses one isolated Worker for those cases and bounded parallel Workers for larger sets. An uncached baseline and the following Candidate wave share one operation-owned executor. Starts and completions remain projected in manifest order even when Workers execute concurrently. CLI projects this beside the shared execution identity on stderr; Studio retains it in the project-local operation registry and polls the same identity independently of page lifetime. Timings and transport envelopes are diagnostic state only and never enter the evaluator verdict or immutable evidence.
