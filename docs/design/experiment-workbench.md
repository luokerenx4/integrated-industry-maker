...
Core also owns one operational `BlueprintBenchmarkProgress` V3 stream. Every baseline and Candidate case has ordered start/completion events, completed/total work, explicit sequential/parallel execution mode and bounded concurrency, cache reuse, and separate compilation, cache-read, simulation, and comparison timings. Parallel case starts and completions are projected in manifest order even though isolated workers execute concurrently. CLI projects this beside the shared execution identity on stderr; Studio retains it in the project-local operation registry and polls the same identity independently of page lifetime. Timings and transport envelopes are diagnostic state only and never enter the evaluator verdict or immutable evidence.
