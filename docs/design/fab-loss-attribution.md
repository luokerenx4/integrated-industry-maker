...
Current compatible run `058-simulate` is the after state. It delivers 45 devices, preserves performance `12/12` and automotive `6/6`, reduces commercial shortfall `10 → 5`, raises portfolio net value `+144 → +164`, reduces rework cycles `6 → 5`, scrap `2 → 1`, and equipment-drift defects `8 → 2`, with no quality escape. The leading quality subject moves from etch to `lithography-1`; both etch bays record zero drift defects. Because the quality intervention removed the preceding top signal instead of freezing the old diagnosis, the current chain is now `q-time → input-starvation → yield-quality → queue-congestion → maintenance-qualification`. Five lots violate Route Q-time in six visits, so both CLI and Studio correctly direct the next investigation there.
