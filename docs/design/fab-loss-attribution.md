...
- verified yield and quality loss: first-pass results over actually inspected unique lots, plus rework, scrap, escapes, and lost lot-derived output; unfinished WIP is not called a quality failure. When equipment drift introduced defects, the bucket also identifies the highest-contributing physical Device and retains exact drifted-lot-job and defect-instance counts.

Each bucket retains exact scalar evidence, a contributor array, and the strongest available Device, connection, Route, or project subject. Non-Q-time buckets expose an empty contributor array rather than inventing detail. Queue congestion prefers the evaluator's productive bottleneck; input starvation ranks the largest `waitingInputTicks × utilization` contribution. This prevents the current fab's eight-second rework exception from outranking sustained burn-in underfeeding simply because the rework bay is correctly empty for most of the window. Zero-signal buckets are omitted. Results are deterministically sorted by descending score and stable id; Q-time contributors sort by visit count, total overrun, then stable id. `primary` is the first bucket and `chain` is the first five bucket ids. Delivery overflow is reported as context but is not itself a loss because above-demand product remains valuable under [[docs/design/delivery-contracts]].
...
`inm inspect --section losses --json` returns the complete structured attribution. The default machine summary includes the outcome, complete primary bucket, chain, and caveat. Human `inm inspect` prints measured Q-time contributor rows even after another signal becomes primary. Studio uses the same object for the Realized Fab Loss Chain panel and renders matching Q-time step, mechanism, Device, lot, and wait evidence without parsing prose or inspecting raw NDJSON.
...
Historical compatible run `057-simulate` delivered 40 devices and left 10 commercial units short. Mean queue time fell from 48.9 to 39.2 seconds and etch became the bottleneck context. More WIP reached inspection, exposing the honest chain `yield-quality → input-starvation → q-time → queue-congestion → batch-formation`: 5 of 12 inspected lots passed first inspection, 6 reworked, and 2 scrapped. `etch-1` was the leading physical quality subject because it introduced 6 of the run's 8 recorded equipment-drift defect instances.
