...
Status: V11 resumable Investigation Design Session over continuous factory observation, source-pinned Candidate and Production-Plan authoring, exact review/TRIAL/comparison/disposition identity, explicit diagnostic-target decisions, and one-variable immutable Run-comparison checkpoints in Core, `inm`, Studio, and the memory-fab north-star fixture.
...
One inspection owns one authoritative Workbench snapshot. Its public result carries the exact project, selection, and evidence hashes from that already-verified snapshot so CLI response metadata does not reopen the project and repeat the full evidence pass. Studio's Design Session bootstrap similarly returns the Workbench overview and its observation brief as two projections of one snapshot, including when the caller selects an immutable Run. This is request-scoped reuse rather than a time-based cache: the next request always observes the filesystem again.
...
A later Investigation decision is also project orientation, not prose that disappears after the Investigation page closes. Workbench V19 resolves the newest decision in the Candidate's own source Investigation whose cited Candidate-review anchor exactly matches the current proposal, base/proposed Blueprint hashes, review result hash, and locked verdict. It projects that entry as `candidate.investigationDisposition`. A current `revise`, `defer`, or `discard` removes a reviewed-KEEP Candidate from pending/apply recommendations and increments the disposed count; a `keep` still leaves explicit application pending. Changed Candidate or review identity makes the old disposition inapplicable rather than allowing historical reasoning to suppress new work. An invalid chain suppresses nothing and remains repairable through Investigation inspection.

A decision may separately target one explicitly cited diagnostic-bearing anchor. Core accepts only `{ kind: "diagnostic", anchorId }` and requires the named anchor to be available at that entry's sequence. Workbench projects it only while the project, complete selection and execution hashes, compatible Run/result, exact diagnostic identity and prose, loss bucket, and leading contributor still reproduce the current snapshot. The newest targeted decision per Investigation wins locally. Across Investigations, any current `revise` keeps the diagnostic active and routes back to that Investigation; only unanimous current `defer`/`discard` evidence suppresses it from the queue. `keep` is visible context without suppression. Changed identity removes the projection automatically, and a broken chain suppresses nothing.
...
- `decision`: an explicit `keep`, `revise`, `defer`, or `discard` judgment and rationale, optionally aimed at one explicitly cited diagnostic-bearing anchor.
...
- a current `blueprint` hypothesis with no exact sourced Candidate yields `author-candidate`;
- one exact unreviewed Candidate yields `review-candidate`;
- one exact reviewed Candidate with no immutable trial yields `simulate-candidate`;
- one exact Candidate TRIAL with no retained control/TRIAL anchor yields `compare-candidate`;
- exact review evidence, or a retained trial comparison, without an authored disposition yields `decide-candidate`;
...
- a completed Candidate `discard` or `defer` cycle requires a fresh current observation before another hypothesis, while `revise` returns to hypothesis authoring and `keep` resumes the shared commissioning boundary.
...
The handoff also carries one `candidateCycle` projection for the newest Blueprint hypothesis. It lists every exactly sourced Candidate and its proposal, review, TRIAL, comparison, and disposition identities. More than one unresolved Candidate is `ambiguous`; stale, corrupt, multiply matching TRIAL, or otherwise broken identity is `invalid`. Core never picks a branch by score, filename, or recency.
...
`inm investigate --create-candidate` is the high-bandwidth authoring path. It consumes an Agent- or human-authored JSON patch file and returns the exact `inm candidate --review` next action without requiring generated hashes. `inm candidate --run` creates the exact immutable TRIAL when the handoff reaches `simulate-candidate`, and `inm compare` opens its parent/TRIAL evidence when the handoff reaches `compare-candidate`. After review, CLI exposes an exact return-to-Investigation action; `--attach-candidate` resolves the receipt, derives `<candidate>-review` when no anchor id is supplied, and adds that introduced evidence to the decision automatically. A decision may pass `--target-diagnostic-anchor <anchor-id>`; CLI automatically cites that anchor and Core validates the exact target. `--capture-observation <anchor-id>` is the post-change continuation boundary: it resolves the current factory checkpoint, adds it to the observation's evidence, and never starts a simulation or accepts a caller-authored hash. `--capture-comparison <anchor-id> --from-run <id> --to-run <id>` retains one already-observed immutable comparison under the same authorship and no-auto-decision boundary. The three evidence-introduction modes are mutually exclusive.
...
Entries `0015–0018` now provide the complete V8 Candidate-cycle fixture. Hypothesis `incumbent-five-performance-seven-commercial` sources the exact Candidate of the same id, its immutable locked review, parent-bound TRIAL Run `109-candidate-trial-incumbent-five-performance-seven`, retained comparison `incumbent-five-seven-comparison`, and explicit `discard` disposition. Reopening the Investigation projects that complete negative result as accumulated evidence and asks for a fresh current observation; removing those artifacts one boundary at a time deterministically resumes `decide-candidate`, `compare-candidate`, `simulate-candidate`, `review-candidate`, and `author-candidate` respectively.

V9 Investigation `current-inspection-starvation-boundary` starts directly from exact Run `105-simulate` and targets its current inspection input-starvation diagnostic. Entry `defer-run-105-inspection-local-branch` records why another inspection-local capacity or transport intervention is not justified: `56.984 s` is fully attributed to upstream processing, waiting, or material already in flight; inspection utilization is `20.7%`; and the factory has only `160` currency of capital headroom. Its explicit `defer` suppresses only that exact diagnostic while those identities remain unchanged. The shared queue therefore advances to the still-current `yield-quality` diagnostic and `layer-two-particle-control`, not directly to Objective WIP; the other historical Design dispositions must be requalified under the current execution contract first.

V10 Investigation `run-105-layer-two-quality` carries that exact quality handoff through the complete human/Agent Candidate cycle. Its observation rejects duplicate capacity and local transport as causes because `etch-l2` is only `31.7%` utilized and accumulates `164.0 s` of input wait with no blocked, unpowered, failed, or utility-wait time. The hypothesis changes both `policy.cadenceControl.normalMode` and its matching normal recipe to `particle-suppression`, preserving `closed-loop-fast-4-5` as the recovery mode. Candidate `run-105-normal-particle-suppression` passes all seven locked outcome guardrails; exact comparison `105-simulate → 110-candidate-trial-run-105-normal-particle-suppress` raises first-pass yield from `83.3%` to `91.7%`, prevents one additional authored defect, removes one rework cycle, and improves the mixed-quality score by `0.546577` without changing output, WIP, cost, area, scrap, or escapes. Entry `keep-normal-particle-suppression` explicitly accepts the small energy-only regressions in three non-target cases and applies the Candidate. Candidate-cycle inspection treats the immutable review anchor as the decision identity after application as well as before it, so the now-verified Candidate remains `completed` instead of asking for a duplicate decision. The current Workbench resumes at Run 110's `input-starvation` diagnostic and `inspection-supply-path`.

V11 Investigation `run-110-furnace-supply` corrects that final handoff. Exact Program focus proves the current `furnace-1` contributor is not the `inspection-1` target. Factory observation `run-110-furnace-factory` binds the compact four-cell deposition-to-furnace path, all twelve deliveries, 1.3% line utilization, zero transport/endpoint loss, 30.0% furnace utilization, 32.1% deposition utilization, and the already installed three-activation fast recovery controller to Run 110 and its exact diagnostic. Entry `defer-run-110-furnace-local-branch` rejects duplicate furnace, faster belt, and repeated threshold search without claiming that the physical starvation vanished. Its current diagnostic-target decision advances Workbench to Probe-owned queue congestion; because the existing queue Program targets `etch-1`, the next action remains observation rather than silently substituting the wrong proposal portfolio.
