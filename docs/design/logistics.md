...
The memory-fab back-end study demonstrates why endpoint stack and power priority are authored equipment state rather than inferred lane tuning. `probe-to-packaging` moved only `24/min` across a nominal `240/min` two-cell lane, yet single-die endpoints produced `46,800` blocked item-ticks. Replacing only those two endpoints with a project-local four-position die-tray handler and setting both to priority eight reduced the exact contributor to zero. The locked Objective still rejected the change because its explicit forty-unit capital and higher active-power cost reduced every current-best case by roughly `0.002`; physical causality and commissioning authority remain separate.
