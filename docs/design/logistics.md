...
This document owns local material movement, sorter/line stages, belt cells and levels, congestion, junction policies, local/inter-zone station routes, and finite carrier fleets.
...
Blocked local item-time is a strict cause/stage partition: line contention, endpoint capacity, endpoint power, and endpoint failure across the line, loader, and unloader. The four immediate-cause totals sum exactly to connection `blockedItemTicks`. Compatible fab-loss contributors expose the dominant cause plus every share; a cell occupied because downstream backpressure propagated upstream remains line contention at that transit, and the engine does not fabricate recursive graph-root attribution.

Every connection reports its authored Resource allowlist, effective dispatch policy, compiled target kind/coverage unit/critical depth for every allowed Resource, plus each stage's explicit Device id, power priority, physical distance and duration, departed/delivered Resource mix, items/min, stack-aware capacity, utilization, average in-flight inventory, loader/unloader utilization, blocked item-ticks, and transport energy. Device metrics preserve sorter idle, processing, unpowered, and failed intervals independently from capacity-normalized stage utilization. Compatible tracked-lot fab loss treats authored transit duration as ordinary cycle-time context and ranks only positive blocked item-ticks; every ranked connection contributor carries the same Resource mix, flow, capacity, utilization, in-flight, and blocked-fraction telemetry. Station analysis records the effective network dispatch policy, configured charging, measured energy ledger, and, for every matched route, mission energy, source/destination slot capacities, reserve/target policy, demand/supply priority, downstream connections, target kind, coverage batch, Objective depth, fleet and energy capacity, load, and deficits. Buffer-contract analysis exposes the same per-Resource quotas used by the simulator.

The memory-fab back-end study demonstrates why endpoint stack and power priority are authored equipment state rather than inferred lane tuning. Current Run 114 moves all 96 dies at `24/min` across a nominal `240/min` adjacent two-cell lane, yet its single-die endpoints accumulate `118,200` blocked item-ticks. Replacing only those two endpoints with project-local four-position die-tray handlers at priority eight reduces the exact contributor to zero. Exact Run `114 → 116` nevertheless preserves delivery, throughput, cycle, WIP, service, and quality while adding forty units of capital, `18,099.5 mJ` energy, and `62,789` unpowered Device-ticks. The human/Agent decision therefore discards the locally smoother design; physical causality and commissioning authority remain separate.
