...
Planned tracked work is declared through `ProductionPlan.lotReleases`, never by placing a tracked Resource count in Scenario `initialBuffers`. Every entry names a stable kebab-case id, Device, buffer, current Resource, required planned release tick, optional integer priority, and optional due tick. Compilation checks identity uniqueness, Resource tracking, physical buffer acceptance, Scenario horizon, and one-lot capacity. A Blueprint may then gate eligible identities through [[docs/design/wip-release-control]] without editing the selected plan. See [[docs/design/production-plans]] and [[docs/design/lot-release-scheduling]].
