...
Status: V2 Core industrial result contract, shared CLI/Studio execution lifecycle, and Investigation-owned Candidate TRIAL operation implemented.
...
Long-running execution has a separate, explicitly non-authoritative Core-owned operation handle. Benchmark, Candidate preview/apply, Investigation Candidate TRIAL, Design run, and Design continuation share one subject/status/progress/artifact/error contract across CLI and Studio. The `candidate-trial` subject pins Investigation id, Candidate id, and seed; Studio admits it only when the current Core handoff is the exact `simulate-candidate` phase for that Candidate. It calls the same `simulateCandidateOperation()` used by `inm candidate --run`, creates or reuses the immutable parent-bound TRIAL Run, and never applies the Candidate. Studio returns a project-qualified operation id immediately and persists its ignored `.inm/operations/<id>/` store as `state.json`, `progress.json`, and, after successful completion, `result.json`. CLI creates the same kind of identity in-process and projects it into progress, success, and failure envelopes without requiring a Studio server. Page navigation and client disconnect only stop Studio observation; they do not cancel industrial work.
...
Benchmark, Candidate preview/apply, Investigation Candidate TRIAL, Design run, and Design continuation start from their domain routes and return `OperationExecutionStartResponse`. `GET /api/projects/<project-id>/operations` reads only the lightweight bounded `state.json` records; complete results and progress logs are fetched from `GET .../operations/<operation-id>` after selecting one exact identity. `DELETE` requests cancellation. Experiments, Investigations, and Design display or follow the operation id and recover the newest exact subject after reopening their stable route. A project refresh reruns that recovery after refreshing authored or immutable project data; it must not leave the workbench empty while the exact retained operation is running or complete. The list bytes and parse work therefore scale with retained operation count, not historical result size.

Experiment and Design routes are lightweight direct surfaces. They use the project index plus `/experiments` or `/designs`, exact Candidate/evidence, and retained-operation endpoints, and they do not require the full Factory event payload, Overview snapshot, or observation brief merely to render one locked evaluation or immutable Design frontier. The full operator workbench is loaded only after the user closes or leaves that surface. This routing difference changes presentation cost only; Benchmark/Design execution, operation identity, progress, result, and Candidate authority remain the shared Core contracts described above.
