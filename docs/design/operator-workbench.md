...
Core derives one visible and machine-readable next action from existing workbench facts so operators do not assign equal weight to every panel. This is an operating projection, not a new industrial conclusion. It selects, in order, the first blocking capacity diagnostic, the first unresolved Candidate decision, missing or incompatible immutable evidence for the exact effective selection, the first flow warning, the latest matching run, or shared analysis. Candidate decisions distinguish a new proposal, an immutable reviewed verdict, an exact reviewed KEEP Blueprint already verified by hash, and a stale base.
