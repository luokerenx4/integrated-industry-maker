...
The next active loss then selects `layer-two-particle-control` because its explicit focus names `yield-quality`. Current Run `47e469f0d22b4d25ff46d89376dfacf1ce70e55f72a5d98743ac7347eafd11a7` switches only `etch-l2` from `closed-loop-control` to `particle-suppression`, reduces the exact origin contributor's `introducedDefectInstances` from `2` to `1`, and improves mixed-quality first-pass yield from `10/12` to `11/12`. It is still rejected: aggregate current-best delta is `-0.451773`, quality-excursion is `-0.861116`, and lithography interruption is `-1.541480` with one fewer on-time lot. The seed remains the sole exhausted frontier node. Workbench therefore adds a second automatically expiring bounded disposition while leaving both physical buckets visible, then advances CLI and Studio to rank-three `queue-congestion` through the broad commissioned brief. That queue diagnostic is now owned by `etch-1 / etch-cell-layer-1`, not the unrelated downstream burn-in rack. Both focused runs remain exact routes and argv away for audit.
