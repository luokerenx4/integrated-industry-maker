...
Catalog-current Run `092-simulate` eventually advances the same queue past rank-eight `transport-blocking`. With all eight realized losses bounded and the Objective capacity plan still `READY`, WIP contributes `-29.8092375`; burn-in package input averages `9.781316666666667` and packaging die input averages `7.965816666666667`. Workbench opens the exact Objective-focused `back-end-wip-convergence` Program. Its authored 5/4 and 4/3 release-wave candidates reduce packaging die-input WIP to `3.764483333333333` and `2.5047`, respectively, but locked service/interruption gates reject them. This is valid bounded design evidence, not permission to auto-apply a lower-WIP release policy.
