...
When several Programs align with the same current Blueprint, Core chooses the shared handoff by evidence state before id: promotable, continuable, exhausted, then missing. This makes durable current authority win over a broader aligned brief that has no current evidence. The ordering changes only which exact Program/result is reopened; it never merges Program portfolios or treats one Program's run as another's evidence.
...
For the current memory fab, `commissioned-dram-fab` and focused `inspection-supply-path` both align with `generated-dram-fab`. The broad Program has no current evidence, while focused Run `c7fbffa625997a667f6e8b831119522e3e94aa666eef103ed65c999c0cf86cee` is current and exhausted. It records six exact shortage-reducing etch/transport interventions, their contributor/metric before-after evidence, five-case rejection authority, unchanged seed, and zero promotion operations. CLI and Studio therefore expose `REVIEW EXHAUSTED DESIGN` and the same focused deep link instead of falling back to the broad missing-evidence brief. A future Program/provider, Benchmark, Blueprint, engine, or catalog change makes this conclusion historical rather than universal.
