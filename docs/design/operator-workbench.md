...
For the current memory fab, strict authority is exhausted Run `459e984f034242d3ddf807592cbd312aa636db0dd066afd3b7305ef74503d137`: one current Design Run and `30` quarantined older artifacts. It evaluated four exact V7 loss-targeted proposals under the strict coverage-deficit cadence contract and retained the unchanged seed with zero promotion operations. The three generic release candidates target queue or release evidence rather than falsely claiming the leading input shortage. CLI and Studio therefore expose `REVIEW EXHAUSTED DESIGN`, the same run id, and the same instruction to add a project-local intervention that matches the observed one-lot rapid-anneal demand and Resource/path/state; neither surface falls back to the generic Program brief or presents invalid artifacts as usable history.
