...
Current `inm-sim/0.93.1` Run `112-simulate` leads with `device:furnace-1:material-input-shortage.starvationTicks = 38,856`. Exact focus prevents `inspection-supply-path` from claiming that contributor, and the broad commissioned Program is not an automatic fallback. Run-110 Investigation evidence records the earlier spatial boundary, but its identity cannot silently dispose the refreshed Run. The shared handoff therefore first opened `/memory-fab/factory?run=112-simulate`; the resulting hash-bearing observation independently reproduced the causal facts and now supports an explicit current defer without hiding the physical loss.

Current factory observations now give both the furnace and Probe contributors exact Run-112 decisions. The Probe checkpoint binds causal hash `f4219c65718af33d585394b9e3975143d3b5cf78c15884969e964250cf084f93`; a current-engine replay of the already authored five-percent Probe acceleration again reduces local queue time but changes `88 → 78` delivered devices and `49.1905 → 52.2903` average WIP. Its explicit `defer` keeps the queue diagnostic visible while removing only further local Probe cycle/input-transport tuning from the active queue. Workbench therefore advances to `layer-two-particle-control` for the still-active yield-quality contributor. The source Investigation may separately remain `repair-evidence` because an older comparison anchor is invalid; scoped current disposition authority comes only from the new exact target, review, and Run-112/113 comparison anchors.
...
Under V9, `maintenance-qualification` no longer routes to `lithography-maintenance-convergence`: current Run `112-simulate` ranks `device:etch-1:maintenance-qualification.readyWorkOverlapTicks = 17000`, while `lithography-1` has zero overlap despite `26000` workload ticks. Historical Run `d45f3aee3f971b689bad074b567646d3ac2f5cf51dc104def9c1dac146752a43` still proves seven-job lithography cadence regresses all five current-best cases and locked service, but its `totalTicks` target cannot impersonate the new etch overlap identity. Maintenance remains visible with workload, overlap, idle context, and exact ready plan rather than reopening a mismatched Program.
