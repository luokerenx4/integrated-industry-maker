...
Status: connected regional grids, proportional satisfaction and priority load shedding, explicit hot/sleep/active Device envelopes, physical wake work, powered equipment changeovers, sorter loads, station charging, high-speed carrier energy, Scenario-driven intermittent renewables and electricity tariffs, fuel generation, deterministic accumulators, interruption-safe Device and sorter-stage work, temporal capacity planning, measured generation/storage research, and joint synthesis implemented through engine version `inm-sim/0.74.0`.

Related: [[docs/design/production-modes]], [[docs/design/logistics]], [[docs/design/equipment-energy-states]], [[docs/design/electricity-tariffs]], [[docs/design/simulation-runtime]], [[docs/design/blueprint-optimization]].
...
At each metric interval and next-boundary decision, runtime builds one same-call grid observation directly from live generation, standby eligibility, active jobs, station charge, transport phase, and blocked transport demand. Metric integration derives generated, active, requested, and transport-share power from that one traversal. Boundary scheduling asks the same observer only for the allocation mode's authoritative load, so it does not compute unused counterfactual or transport-breakdown fields. Stable storage and station-energy membership/order are prepared topology; health, stored energy, charge satisfaction, and every power quantity remain live. The observation is discarded before state mutation and is never a cached ledger, epoch, or invalidation authority.
...
Grid membership, authored priority, Device identity, and the Device/kind combinations that can physically become active consumers are compiled for the duration of one simulation. Runtime allocation traverses that prepared sequence while reading generation, storage, failures, sleep state, jobs, charging, and transport activity live. A generic project-local Device Program may start work without a bound Process, so job possibility follows the permanent Device-evaluation boundary rather than only compiled recipe membership. Priority active consumers retain their exact total order as Device rank followed by the fixed kind rank `job`, `station-charge`, `transport`; the simulator must not rebuild a sorted consumer projection or cache mutable eligibility.
...
Compatible tracked-lot loss attribution groups every positive `unpoweredTime` value into a deterministic Device contributor, then uses events to distinguish lost active service from standby shedding. Ordered active-job and sorter-stage shortage/restoration pairs close at the Run boundary when still open. `serviceInterruptionTicks` is the exact active-job plus active-transport interval and owns loss score/ranking; `standbyUnpoweredTicks` is the conserved residual context. Their sum must equal evaluator-owned `unpoweredTime` per Device and factory-wide. A contributor also carries the exact compiled grid, optional sorter connection and loader/unloader stage, shortage/restoration counts, Device peak demand deficit, affected distributor Device subjects, and the grid energy metrics above. This exposes where power denied actual production service without pretending that idle infrastructure telemetry is recovered output, or that generation, storage, reprioritization, or accepting the interruption is automatically the right commercial decision.

Scenario-owned regional tariffs value actual served plant load at exact time-of-use boundaries and aggregate simultaneous metered power across every grid in a region for one peak-demand charge. The tariff is fixed evaluator input; sleep, scheduling, storage, generation, and equipment topology remain Blueprint decisions. See [[docs/design/electricity-tariffs]].
