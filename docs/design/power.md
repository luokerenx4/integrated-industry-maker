...
Grid membership, authored priority, and Device identity are compiled for the duration of one simulation. Runtime allocation traverses that precomputed Device order while reading generation, storage, failures, sleep state, jobs, charging, and transport activity live. Priority active consumers retain their exact total order as Device rank followed by the fixed kind rank `job`, `station-charge`, `transport`; the simulator must not rebuild a sorted consumer projection or cache mutable eligibility.
