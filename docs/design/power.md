...
Compatible tracked-lot loss attribution groups every positive `unpoweredTime` value into a deterministic Device contributor. A contributor carries the exact compiled grid, optional sorter connection and loader/unloader stage, shortage/restoration counts, Device peak demand deficit, affected distributor Device subjects, and the grid energy metrics above. Contributor time must conserve the evaluator-owned aggregate exactly. This exposes where power was denied without claiming that additional generation, storage, reprioritization, or accepting the interruption is automatically the right commercial decision.
