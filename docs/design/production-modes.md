...
The commissioned Design provider may propose the exact one-field mode switch when current compatible evidence ranks `input-starvation`. Current V5 Design Run `0ad66de96d35b9a126331acb0e8e7cd81c5b4e8becec8345d13c4fd6d65706c1` retains that proposal only as a non-dominated `BRANCH`: it improves the weighted five-case score by `0.691655`, but loses `0.375853` in steady production and `0.923040` under facility interruption against the current best. The zero-regression leader boundary therefore emits no Candidate and leaves the live factory on `qualified`.
