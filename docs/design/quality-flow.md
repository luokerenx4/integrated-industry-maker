...
Status: deterministic authored process excursions, mode-level in-situ prevention, latent lot defects, inline inspection, selective rework, terminal scrap, quality metrics, and Blueprint benchmarking implemented through engine version `inm-sim/0.93.1`; V10 fab-loss attribution gives every observed origin defect class a separate conserved contributor identity.
...
Focused historical Program `layer-two-particle-control` repeats that physical question against the later commissioned factory instead of treating the old branch as permanent authority. Its V10 target ends in `:particle-contamination`, so it can never claim a critical-dimension origin merely because both occur on the same Device and Process. The selected `particle-suppression` mode eventually became commissioned evidence and left Run `112-simulate` with eleven first-pass lots, one rework, and exact `3 authored / 2 prevented / 1 applied` quality accounting.

That remaining applied instance is the critical-dimension excursion on `dram-lot-03`. Program `layer-two-dimensional-control` binds its full V10 contributor identity and proposes a distinct `dimensional-stability` mode that prevents critical-dimension, particle-contamination, and latent-electrical excursions at `3/2` active power without shortening the challenged Process. Candidate `run-112-dimensional-stability` improves aggregate current-best score by `0.253889`; mixed-quality gains `0.5164645`, systematic quality gains `0.1180288`, and lithography interruption gains `0.5152895`. Steady and facility cases pay only `0.004700` and `0.002350`, and none of the seven locked guardrails crosses its limit.

Applied trial Run `114-candidate-trial-run-112-dimensional-stability` confirms the intervention outside proposal scoring: all twelve lots finish first pass, all 88 devices are still delivered, average Objective-equivalent WIP remains `49.1905`, and all three authored defect instances are prevented before they enter lot state. The comparison and explicit KEEP receipt close the dimensional-control Investigation; subsequent furnace observation belongs to a new question rather than being appended to the solved quality narrative.
