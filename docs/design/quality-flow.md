...
Focused current Program `layer-two-particle-control` repeats that physical question against the later commissioned factory instead of treating the old branch as permanent authority. Design Run `220378460b16c5eefdf12ef787b4e494ba810ee9b56da010c5ef7596978c7190` records the exact current target `introducedDefectInstances 2 → 1`; mixed-quality improves to eleven first-pass lots and one rework. The whole-fab decision is now worse rather than marginally positive: aggregate current-best delta is `-0.451773`, systematic quality is `-0.861116`, lithography interruption is `-1.541480` with eight rather than nine on-time lots, and steady/facility cases pay the authored energy increment. The immutable result exhausts the single eligible intervention, creates no Candidate, and supports a hash-current bounded disposition without claiming the defect source disappeared.
