...
Focused current Program `layer-two-particle-control` repeats that physical question against the later commissioned factory instead of treating the old branch as permanent authority. Design Run `47e469f0d22b4d25ff46d89376dfacf1ce70e55f72a5d98743ac7347eafd11a7` records the exact current V8 target `introducedDefectInstances 2 → 1`; mixed-quality improves to eleven first-pass lots and one rework. The whole-fab decision is now worse rather than marginally positive: aggregate current-best delta is `-0.451773`, systematic quality is `-0.861116`, lithography interruption is `-1.541480` with eight rather than nine on-time lots, and steady/facility cases pay the authored energy increment. The immutable result exhausts the single eligible intervention, creates no Candidate, and supports a hash-current bounded disposition without claiming the defect source disappeared.
