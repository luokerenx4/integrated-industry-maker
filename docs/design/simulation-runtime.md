...
Contract-value scheduling likewise compiles observation topology rather than mutable evidence. Per contract region, the runtime prepares exact Resource-specific resident Buffer, possible active-output Device, inbound local-connection, and station-network sources. Each eligible Device evaluation still constructs one same-state commitment snapshot from current delivery metrics, Buffer quantities, active jobs, and transit arrays. The compiled source index owns no count, cannot become stale, and does not change the existing per-Device snapshot boundary.
