...
`GET /api/projects/<project-id>/overview` returns the exact Core [[docs/design/operator-workbench]] snapshot used by `inm inspect --json`. Optional World, Blueprint, Scenario, and Objective query selectors are explicit and never fall back when invalid. The endpoint is read-only and does not select a run, execute a Benchmark, preview a Candidate, or create Studio state. The project Overview renders its selection and hashes, Objective/contracts, target-rate readiness, prioritized active diagnostics, recent immutable evidence, proposals, and operation descriptors. A Workbench V8 bounded disposition appears as a dedicated evidence panel with its exact target/value, candidate counts, locked decision bases, invalidation boundary, and route to the source Design Run. The physical bucket remains in the realized-loss chain with a `BOUNDED DEFERRED` label, while only its exact diagnostic is removed from the active queue. The richer `/data` endpoint separately supplies Factory replay data and its selected immutable run.
