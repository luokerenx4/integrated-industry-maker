...
Project-file refresh notifications use the project server's `/api/watch` WebSocket. Multiple Studio tabs may hold independent watch sockets without consuming the browser's bounded pool of ordinary HTTP/1.1 request connections, so project index, Overview, data, and asset requests remain available. The client reconnects a dropped watch socket while the page remains mounted and closes it on unmount; the server publishes only the same filtered project-file changes used by the filesystem watcher.
