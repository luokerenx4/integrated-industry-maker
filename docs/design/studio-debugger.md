...
Two selectors create one copied `/<project-id>/runs?from=<id>&to=<id>` route. `GET /api/projects/<project-id>/run-comparison?from=<id>&to=<id>` returns the Core `compareFactoryRuns` object unchanged. It either rejects the pair with typed identity/compatibility evidence or renders exact result hashes, semantic/spatial changes, score/cost/area/movement deltas, delivery/quality/capacity/constraint guardrails, changed fab-loss scores and leaders, and the Core verdict. Every changed Device or Connection links to both exact historical Factory routes. `RETAIN IN INVESTIGATION` carries the exact FROM/TO ids to the Investigation list without writing; selecting or creating an Investigation preserves that pair and pre-fills an explicit observation capture. The page states the design boundary explicitly: comparison explains observed evidence and never recommends or applies an intervention.
...
Creating freezes current compatible evidence. Appending requires a chosen human/Agent author and kind; hypothesis and decision fields enforce their additional expected-effect or disposition boundary. The form may name one reviewed Candidate plus a new evidence-anchor id, capture the current factory observation, or capture the exact Run pair supplied by the comparison route; those modes are mutually exclusive and only an observation may introduce operating evidence. Core resolves the Candidate receipt or Run identities, comparison hash, TO context, and diagnostic, and the introducing entry may cite the new anchor immediately. The server delegates every write and currentness check to Core, then returns the complete shared inspection projection. Opening, reloading, selecting, or following an evidence link never manufactures prose or edits industrial state.
