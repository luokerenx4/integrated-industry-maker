...
Current compatible Run `090-simulate` supplies the causal maintenance picture. Its four maintained production Devices conserve `94,000` service + `30,000` qualification ticks with zero input or crew wait. Both lithography generations own `34,000` ticks and both etch generations own `28,000`; observed phase events bind all four to `maintenance-service-1`. The dual-crew provider completes all sixteen assignments at peak `2/2`, so increasing service capacity is not a supported response.

The deliberate follow-up changes only `lithography-1` planned cadence from six to seven jobs. Focused Design Run `630b46d261c21a6c31a39d1d0ea345eebdc73d2100224e64fb01eac2fd27dde2` confirms the local trade: its exact contributor falls `34,000 → 17,000` ticks and one service/qualification kit is saved, but one drifted job introduces one defect. Aggregate score falls `-3.342370`; every locked current-best case regresses, the lithography-interruption case falls `-9.907084`, and steady, mixed-quality, and facility-interruption cases fail the locked on-time-service outcome. The unchanged six-job policy remains commissioned and the rejected Candidate is retained as bounded evidence, not automatically applied.
