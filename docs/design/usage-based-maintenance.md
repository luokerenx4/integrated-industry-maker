...
- While service, qualification, or an explicit provider wait owns the Device, the runtime continuously evaluates the same qualified Process plans that production would use. An overlap interval exists only while at least one exact Process/mode plan has every material input, tracked-lot input, output slot, reusable tool, and utility allocation physically ready. A change in phase, wait reason, or ready-plan set closes one interval and opens another.

The event stream distinguishes phase-aware blocking, service start/finish/cancellation, qualification start/finish/cancellation, whole-cycle completion, process drift, and ready-work interval start/finish. Every maintenance event carries exact `opportunistic`, `planned-boundary`, or `asset-limit` cause plus `usage` or `calendar` trigger attribution and the captured qualification age. Phase finish/cancellation events retain both nominal contract duration and actual wall-clock occupied duration, so power stretching and partial cancellation remain physical evidence. Metrics retain disjoint service and qualification occupied time, exact consumable/crew waits, phase-specific ready-work overlap, interval count, and `readyWorkOverlapTicks + idleWindowTicks = service + qualification + provider waits` per Device and factory-wide. Provider service and qualification crew-time are disjoint as well.
...
The historical planned-stop commissioning study used the distinct semantics directly. It selected a stop after six completed jobs and Design Run `83f7355c1122bfb704c14d59cb693ed46251b8cd19b36dbedba77afd38c124e8` promoted the resulting patch through Candidate `planned-lithography-maintenance`. Compatible run `065-simulate` recorded zero drift defects, nine completed and nine first-pass-completed lots, three terminal scrap dispositions, one Q-time visit, portfolio net value `196`, and explicit completion attribution of two asset-limit, two planned-boundary, and six opportunistic cycles. The checked-in `memory-fab:research-planned-maintenance` script always compares against the current incumbent; its current six-job row is therefore the unchanged reference, not a replay of the old commissioning delta.

Current compatible Run `112-simulate` supplies the corrected causal maintenance picture under `inm-sim/0.93.1`. Four maintained production Devices conserve `64,000` service + `30,000` qualification ticks with zero input or crew wait. Only `23,143` ticks overlap a complete physically ready Process plan; `70,857` occur in idle or post-production windows. `etch-1` is the true leader with `17,000` overlap ticks for `etch-cell-layer-1 / qualified`, while `lithography-1` owns `26,000` physical workload ticks but zero ready-work overlap. The dual-crew provider completes all sixteen assignments at peak `2/2` and conserves disjoint `64,000` service plus `30,000` qualification crew-ticks, so neither more service capacity nor raw lithography workload is supported as the next response.

The earlier deliberate follow-up changed only `lithography-1` planned cadence from six to seven jobs. Focused Design Run `630b46d261c21a6c31a39d1d0ea345eebdc73d2100224e64fb01eac2fd27dde2` correctly rejected it: one drifted job introduced one defect, aggregate score fell `-3.342370`, every locked current-best case regressed, and three cases failed the on-time-service outcome. Its old `34,000 → 17,000` total-work target is now explicitly historical rather than current loss authority. The unchanged six-job policy remains commissioned; a future etch intervention must begin from the new exact overlap evidence and still clear the locked industrial cases.
