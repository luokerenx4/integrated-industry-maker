...
Historical compatible run `090-simulate` first made the remaining controller hold causal: six lots owned all `171,738` blocked lot-ticks, physical capacity owned zero, and `dram-lot-07` led with `63,623` ticks. Focused Design Run `1a23962af0674431da235210d017fa8cba39c296ecca06d4f61ff2e5a67ed49d` tested only the smallest one-card intervention, `7/6 EDD`. It eliminated that exact contributor but regressed all five locked current-best cases and reduced facility-interruption on-time service from nine lots to eight. This remains the historical boundary, not current authority after later Blueprint and causal identities changed.

Current Run `114-candidate-trial-run-112-dimensional-stability` independently requalifies that judgment under the commissioned no-rework flow. Six lots now conserve `162,138` controller ticks and zero physical-capacity ticks; `dram-lot-07` owns `62,023`, enters twelfth at tick `98,023`, and still has `81,977` due-date-slack ticks. Investigation-sourced Candidate `run-114-one-card-release-window` again changes only `6/5 → 7/6`. Trial Run `115-candidate-trial-run-114-one-card-release-window` removes exactly the lot-07 hold and lowers total admission wait to `100,115` ticks, but delivery remains `88`, average WIP rises `49.1905 → 51.257933`, mean lot cycle rises by `4,248.083` ticks, final packaged WIP moves from lot 07 to lot 08, every current case regresses, and facility-interruption on-time service again falls `9 → 8`. The explicit Agent `discard` retains `6/5 EDD`; the loss stays measured as intentional WIP/service protection rather than being renamed solved waste.
