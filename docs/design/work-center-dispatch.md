...
- `contract-value`: greatest evaluator-owned delivery contribution achievable over the remaining Scenario contract window after productive work and any direct physical delivery handoff; equal window value prefers earlier customer delivery, then one-job marginal value per process tick;
...
`cadenceControl` is a separate, deliberately narrower controller for exactly two modes of one Process with identical material work. It selects the recovery mode only after resident plus in-flight coverage on one exact downstream Connection has remained below the authored item boundary for the configured coverage-deficit interval. This predictive pressure signal is distinct from event-backed downstream input starvation. It cannot coexist with `recipeDispatch`, `setupCampaign`, or `batchFormation`; broader operation choice remains owned by those policies rather than an implicit priority stack.

Selection occurs only while the Device is idle. A ready operation has every exact input batch available and enough reserved output capacity. If its Process setup group differs from setup-sensitive equipment state, the host may first apply the Blueprint's bounded setup-campaign rule, then completes the asset's exact directed powered transition before material is consumed. Work is non-preemptive after start. When nothing is ready, the highest-ranked operation is still exposed to the Device program so the normal waiting-input or blocked-output state remains observable. The deliberate exception is a `contract-value` Device after every qualified operation has zero realizable delivery value inside the remaining Scenario window: no fallback Process is exposed, so the factory does not consume WIP or power on a batch that cannot cross its explicit customer boundary in time. See [[docs/design/equipment-changeover]] and [[docs/design/setup-campaign-control]].
...
For `contract-value`, the runtime projects the number of complete cycles whose output can reach a direct consuming delivery Device before the Scenario contract window ends. Each contracted output pays the exact currently required directed setup transition, productive duration, and its own compiled loader/line/unloader `travelTicks`; coproducts may therefore have distinct delivery horizons. It values only those deliverable outputs against committed demand, then uses first customer-delivery time, one-job value rate, authored priority, and authored order as deterministic tie-breaks. If a contract output has no direct consuming Connection, the projection retains the historical zero-lead local boundary rather than inventing an unmodeled route. This remains a local equipment-capacity horizon: it assumes continuing feed for the projection, while ordinary readiness still requires the next exact material batch, tooling, utility capacity, output room, and power before physical work starts. See [[docs/design/delivery-contracts]].
