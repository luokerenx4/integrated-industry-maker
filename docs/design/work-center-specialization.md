...
Ground-only greedy routing improved aggregate performance but created a long detour and failed the per-case gate. Adding the already physical elevated-routing degree of freedom produced a compact dedicated layer-2 line. Finite fab facilities then exposed a second-order constraint: the dedicated tools overloaded one shared utility plant. The kept Blueprint also purchases a second plant and now scores `94.771430` across five end-to-end product cases, improves the weighted aggregate by `238.160724` and every case by at least `200.893383`, and remains under the Objective's build-cost and occupied-area constraints. The fifth case fails the primary plant and demonstrates that the second plant is N+1 capacity for subsequent starts, not only normal-production throughput. Earlier pre-utility rankings are historical evidence, not scores that can be compared across engine physics.
