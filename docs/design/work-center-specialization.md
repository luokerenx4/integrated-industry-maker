...
Ground-only greedy routing improved aggregate performance but created a long detour and failed the per-case gate. Adding the already physical elevated-routing degree of freedom produced a compact dedicated layer-2 line. Under the current finite-reticle, active-job interruption, drift, maintenance, and post-service qualification physics, the kept Blueprint scores `-65.528078`, improves every case by at least `26.248400`, and remains under the Objective's build-cost and occupied-area constraints. The earlier pre-qualification ranking is historical evidence, not a score that can be compared across engine physics.
