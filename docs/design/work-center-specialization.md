...
Ground-only greedy routing improved aggregate performance but created a long detour and failed the per-case gate. Adding the already physical elevated-routing degree of freedom produced a compact dedicated layer-2 line. Together with the subsequently selected drift- and service-aware maintenance policy, the kept Blueprint raises the current locked aggregate score from `-45.952297` to `-13.763165`, improves every case by at least `18.502208`, and remains under the Objective's build-cost and occupied-area constraints.
