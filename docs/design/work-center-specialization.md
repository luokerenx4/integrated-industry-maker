...
The heterogeneous deep+rapid topology removes final-inspection Q-time violations but totals `140,760`, so the evaluator rejects it before score trade-offs. Replacing deep inspection with one rapid-only cell is comfortably feasible at `117,350` and is the best new alternative at `30.540497`, but it remains `1.225340` behind the `31.765837` incumbent because faster flow no longer detects latent electrical defects. Two rapid cells score still lower after capital and area costs. Across 56 equipment, maintenance, and lot-dispatch variants no architecture clears the locked gate, so `bun run memory-fab:research-metrology` preserves the incumbent. Physical throughput improvement is insufficient unless qualification coverage, the whole operating envelope, and capital constraints improve together.
