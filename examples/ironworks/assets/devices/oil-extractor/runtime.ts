...
export default {
  ...
  evaluate(context) {
    ...
    return { kind: "extract", operation: `extract-${node.resource}`, durationTicks: extraction.cycleTicks, node: node.id, count: Math.min(extraction.itemsPerCycle, node.remaining) };
  },
