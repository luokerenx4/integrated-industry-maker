...
export async function analyzeCommand(projectDir: string, selection: ProjectSelection, options: OutputOptions): Promise<void> {
  ...
  const lines = [
    ...
    ...analysis.productionGraph.steps.map((step) => `  ${step.device.padEnd(24)} ${step.process.padEnd(20)} ${step.cyclesPerTarget.toFixed(3)} cycles`),
    "",
    "Recipe alternatives",
    ...analysis.recipeOptions.filter((option) => !option.selected).map((option) => `  ${option.device.padEnd(24)} ${option.process.padEnd(20)} ${option.targetOutputPerMinute.toFixed(3)} ${project.objective.targetResource}/min  ${Object.entries(option.inputBindings).map(([resource, buffer]) => `${resource}→${buffer}`).join(" + ")} ⇒ ${Object.entries(option.outputBindings).map(([resource, buffer]) => `${resource}→${buffer}`).join(" + ")}`),
