  ...
  write([
    ...
    ...result.cases.flatMap((item) => [
      ...
      ...(item.baselineMetrics.completedLots || item.candidateMetrics.completedLots ? [
        ...
        `    utilities ${item.baselineMetrics.totalUtilityAllocations} jobs / ${item.baselineMetrics.totalUtilityCompleted} complete / ${item.baselineMetrics.totalUtilityCancelled} cancelled / ${(item.baselineMetrics.totalUtilityUnitTicks / 1000).toFixed(3)} capacity-s / ${(item.baselineMetrics.totalUtilityInputWaitTicks / 1000).toFixed(3)} s wait → ${item.candidateMetrics.totalUtilityAllocations} / ${item.candidateMetrics.totalUtilityCompleted} / ${item.candidateMetrics.totalUtilityCancelled} / ${(item.candidateMetrics.totalUtilityUnitTicks / 1000).toFixed(3)} / ${(item.candidateMetrics.totalUtilityInputWaitTicks / 1000).toFixed(3)}`,
        `    maintenance ${item.baselineMetrics.totalMandatoryMaintenance} mandatory / ${item.baselineMetrics.totalOpportunisticMaintenance} opportunistic / ${(item.baselineMetrics.totalMaintenanceTicks / 1000).toFixed(3)} s → ${item.candidateMetrics.totalMandatoryMaintenance} / ${item.candidateMetrics.totalOpportunisticMaintenance} / ${(item.candidateMetrics.totalMaintenanceTicks / 1000).toFixed(3)} s`,
