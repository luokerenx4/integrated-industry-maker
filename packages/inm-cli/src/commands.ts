...
export async function simulateCommand(projectDir: string, selection: ProjectSelection, options: { seed: number; untilTick?: number; maxEvents?: number; json: boolean }): Promise<void> {
  ...
  if (options.json) write(summary, true);
    ...
    ...(result.metrics.lotFlow.family ? [
      ...
      ...Object.entries(result.metrics.routeFlow).map(([route, flow]) => `Route ${route}: ${flow.transitions} transitions · ${flow.reentrantTransitions} re-entrant · ${flow.completed} complete · ${flow.scrapped} scrap · ${flow.inProgress} in progress`),
    ...
    `Equipment maintenance: ${result.metrics.equipmentMaintenance.totalCompleted} completed (${result.metrics.equipmentMaintenance.totalMandatory} mandatory / ${result.metrics.equipmentMaintenance.totalOpportunistic} opportunistic) · ${result.metrics.equipmentMaintenance.totalCancelled} cancelled · ${(result.metrics.equipmentMaintenance.totalMaintenanceTicks / 1000).toFixed(3)} s work${Object.entries(result.metrics.equipmentMaintenance.devices).length ? ` · ${Object.entries(result.metrics.equipmentMaintenance.devices).map(([device, maintenance]) => `${device}=${maintenance.jobsSinceMaintenance} jobs`).join(", ")}` : ""}`,
