...
export async function analyzeCommand(projectDir: string, selection: ProjectSelection, options: OutputOptions): Promise<void> {
  ...
  const lines = [
    ...
    "",
    "Device rates",
    ...analysis.extractionDevices.map((device) => `  ${device.device.padEnd(24)} extract ${device.resource.padEnd(15)} ${device.itemsPerMinute.toFixed(3)} items/min from ${device.nodes.join(", ")} · ${(device.idlePowerMilliWatts / 1000).toFixed(3)} W idle / ${(device.powerMilliWatts / 1000).toFixed(3)} W active`),
    ...analysis.devices.flatMap((device) => [
      `  ${device.device.padEnd(24)} ${`${device.process}/${device.mode}`.padEnd(32)} ${device.cyclesPerMinute.toFixed(3)} jobs/min · ${(device.idlePowerMilliWatts / 1000).toFixed(3)} W idle / ${(device.powerMilliWatts / 1000).toFixed(3)} W active${device.minimumInputTreatmentLevel ? ` · inputs @${device.minimumInputTreatmentLevel}+` : ""}`,
