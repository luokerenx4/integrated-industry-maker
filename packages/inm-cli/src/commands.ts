...
function cadenceControlSide(
  ...
  const boundary = control.kind === "downstream-coverage-recovery"
    ? `recover after ${(control.minimumCoverageDeficitTicks / 1000).toFixed(1)}s below ${control.recoverBelowItems} items on ${control.downstreamConnection} · ${control.coverageDeficitEpisodes} coverage-deficit episodes / ${(control.coverageDeficitTicks / 1000).toFixed(1)}s`
    : `recover after the oldest ${control.inputResource} lot waits ${(control.minimumQueueTicks / 1000).toFixed(1)}s with ${control.recoverAtItems}+ resident items`;
  return `${control.normalJobs} ${control.normalMode} / ${control.recoveryJobs} ${control.recoveryMode} jobs · ${control.recoveryActivations} recovery activations · ${control.process} · ${boundary}`;
}
...
export async function inspectCommand(projectDir: string, selection: ProjectSelection, options: OutputOptions): Promise<void> {
  ...
  const setupContributors = snapshot.lossAttribution?.buckets
    .find((bucket) => bucket.id === "setup-campaign")?.contributors ?? [];
  ...
  if (options.json) {
    ...
    ...(snapshot.lossAttribution?.primary ? [
      ...
      ...(setupContributors.length ? [
        "Setup, changeover, and campaign contributors:",
        ...setupContributors.slice(0, 7).map((contributor) =>
          `  ${contributor.label} · ${contributor.mechanism} · ${(contributor.evidence.totalTicks! / 1000).toFixed(1)}s · ${contributor.processes.join("+") || "unresolved process"} · ${contributor.resources.join("+") || "no material context"} · ${contributor.evidence.powerMilliWatts}mW / ${contributor.evidence.energyMilliJoules}mJ · ${contributor.releaseCause ?? "physical transition"} · ${contributor.subjects.map((subject) => `${subject.kind}:${subject.id}`).join(" → ")}`),
        ...(setupContributors.length > 7 ? [`  … ${setupContributors.length - 7} more in --section losses --json`] : []),
      ] : []),
      ...(maintenanceContributors.length ? [
        ...
        ...maintenanceContributors.slice(0, 5).map((contributor) => {
          ...
          const qualificationConsumables = Object.entries(contributor.consumables.qualification)
            .map(([resource, count]) => `${count} ${resource}`).join(" + ") || "none";
          const readyPlans = contributor.readyWorkPlans
            ?.map((plan) => `${plan.process}/${plan.mode}`).join(" + ") || "no observed ready operation";
          return `  ${contributor.label} · ${(contributor.evidence.readyWorkOverlapTicks! / 1000).toFixed(1)}s ready-work overlap / ${(contributor.evidence.idleWindowTicks! / 1000).toFixed(1)}s idle-window context · ${(contributor.evidence.totalTicks! / 1000).toFixed(1)}s workload = ${(contributor.evidence.maintenanceTicks! / 1000).toFixed(1)}s service + ${(contributor.evidence.qualificationTicks! / 1000).toFixed(1)}s qualification + ${(contributor.evidence.inputWaitTicks! / 1000).toFixed(1)}s input wait + ${(contributor.evidence.crewWaitTicks! / 1000).toFixed(1)}s crew wait · ${readyPlans} · ${contributor.evidence.assetLimit} asset / ${contributor.evidence.plannedBoundary} planned / ${contributor.evidence.opportunistic} opportunistic · ${contributor.evidence.usageTriggered} usage / ${contributor.evidence.calendarTriggered} calendar · service ${serviceConsumables} · qualification ${qualificationConsumables} · ${devices}`;
        }),
      ...
      ...(powerContributors.length ? [
        ...
        ...powerContributors.slice(0, 7).map((contributor) =>
          `  ${contributor.label} · ${contributor.grid ?? "unbound grid"}${contributor.endpointStage ? ` / ${contributor.endpointStage}` : ""} · ${(contributor.evidence.serviceInterruptionTicks! / 1000).toFixed(1)}s active service (${(contributor.evidence.serviceInterruptionShare! * 100).toFixed(1)}%) = ${(contributor.evidence.activeJobInterruptionTicks! / 1000).toFixed(1)}s job + ${(contributor.evidence.transportInterruptionTicks! / 1000).toFixed(1)}s transport · ${(contributor.evidence.standbyUnpoweredTicks! / 1000).toFixed(1)}s standby context · ${(contributor.evidence.unpoweredTicks! / 1000).toFixed(1)}s total unpowered · device peak ${contributor.evidence.peakDeficitMilliWatts}mW · grid ${contributor.evidence.gridUnservedMilliJoules}mJ unserved / ${contributor.evidence.gridPeakDeficitMilliWatts}mW peak / ${contributor.evidence.gridRequiredStorageCapacityMilliJoules}mJ envelope · ${contributor.subjects.map((subject) => `${subject.kind}:${subject.id}`).join(" → ")}`),
...
export async function analyzeCommand(projectDir: string, selection: ProjectSelection, options: OutputOptions): Promise<void> {
  ...
  const lines = [
    ...
    "",
    "Configured material envelope [DESCRIPTIVE ONLY]",
    `  ${analysis.configuredEnvelope.summary}`,
    ...
    "",
    "Power grids [SIMULTANEOUS RATED ENVELOPE · DESCRIPTIVE ONLY]",
...
export async function compareCommand(
  ...
  write([
    ...
    ...(fromMetrics.completedLots || toMetrics.completedLots ? [
      ...
      `  release mean delay ${(fromMetrics.meanReleaseDelayTicks / 1000).toFixed(3).padStart(12)} → ${(toMetrics.meanReleaseDelayTicks / 1000).toFixed(3).padStart(12)} s  Δ ${signed(comparison.delta.meanReleaseDelayTicks / 1000)} s`,
      `  release max delay  ${(fromMetrics.maximumReleaseDelayTicks / 1000).toFixed(3).padStart(12)} → ${(toMetrics.maximumReleaseDelayTicks / 1000).toFixed(3).padStart(12)} s  Δ ${signed(comparison.delta.maximumReleaseDelayTicks / 1000)} s`,
      ...
      `  protected releases ${fromMetrics.serviceProtectedReleases.toFixed(0).padStart(12)} → ${toMetrics.serviceProtectedReleases.toFixed(0).padStart(12)}  Δ ${signed(comparison.delta.serviceProtectedReleases, 0)}`,
      ...
      `  prevented/authored ${`${fromMetrics.preventedDefectInstances}/${fromMetrics.authoredDefectInstances}`.padStart(12)} → ${`${toMetrics.preventedDefectInstances}/${toMetrics.authoredDefectInstances}`.padStart(12)} defects`,
...
export async function simulateCommand(projectDir: string, selection: ProjectSelection, options: { seed: number; untilTick?: number; maxEvents?: number; json: boolean; section?: string }): Promise<void> {
  ...
  if (options.json) {
    ...
    summary: () => ({ cached: summary.cached, run: summary.run, resultHash: summary.resultHash, runKey: summary.runKey, metrics: {
      ...
      inventoryAccounting: result.metrics.inventoryAccounting, cadenceControl: result.metrics.cadenceControl,
    } }),
    ...
    ...(result.metrics.lotFlow.family ? [
      ...
      `Release control: ${result.metrics.releaseFlow.control}${result.metrics.releaseFlow.maximumWip === null ? "" : ` max ${result.metrics.releaseFlow.maximumWip} / reopen ${result.metrics.releaseFlow.reopenAtWip} / ${result.metrics.releaseFlow.dispatch}${result.metrics.releaseFlow.serviceLevelAfterTicks === null ? "" : ` / service after ${(result.metrics.releaseFlow.serviceLevelAfterTicks / 1000).toFixed(3)} s`}`} · ${result.metrics.releaseFlow.peakActiveLots} peak active · ${result.metrics.releaseFlow.controlBlockedLots} control-blocked lots / ${(result.metrics.releaseFlow.controlBlockedTicks / 1000).toFixed(3)} lot-s · ${result.metrics.releaseFlow.serviceLevelOpenings} service openings / ${result.metrics.releaseFlow.serviceProtectedReleases} protected releases`,
      ...
      ...(result.metrics.sourceLotLineage.sourceLots.length ? [
        ...
        ...result.metrics.sourceLotLineage.sourceSets.flatMap((entry) => entry.finalWip.map((location) =>
          `  ${entry.sourceLotIds.join(" + ")} → ${location.count} ${location.resource} at ${describeWipInventoryLocation(location)} (${location.kind})`)),
      ] : []),
    ...
    `Equipment maintenance: ${result.metrics.equipmentMaintenance.totalCompleted} released (${result.metrics.equipmentMaintenance.totalAssetLimit} asset-limit / ${result.metrics.equipmentMaintenance.totalPlannedBoundary} planned-boundary / ${result.metrics.equipmentMaintenance.totalOpportunistic} opportunistic · ${result.metrics.equipmentMaintenance.totalUsageTriggered} usage / ${result.metrics.equipmentMaintenance.totalCalendarTriggered} calendar) · ${result.metrics.equipmentMaintenance.totalCancelled} service cancelled · ${(result.metrics.equipmentMaintenance.totalMaintenanceTicks / 1000).toFixed(3)} s service / ${(result.metrics.equipmentMaintenance.totalQualificationTicks / 1000).toFixed(3)} s qualification / ${(result.metrics.equipmentMaintenance.totalInputWaitTicks / 1000).toFixed(3)} s consumable wait / ${(result.metrics.equipmentMaintenance.totalCrewWaitTicks / 1000).toFixed(3)} s crew wait · ${(result.metrics.equipmentMaintenance.totalReadyWorkOverlapTicks / 1000).toFixed(3)} s ready-work overlap / ${(result.metrics.equipmentMaintenance.totalIdleWindowTicks / 1000).toFixed(3)} s idle-window context / ${result.metrics.equipmentMaintenance.totalReadyWorkIntervals} intervals · ${(result.metrics.equipmentMaintenance.totalServiceCrewTicks / 1000).toFixed(3)} service crew-s / ${(result.metrics.equipmentMaintenance.totalQualificationCrewTicks / 1000).toFixed(3)} qualification crew-s · service ${Object.entries(result.metrics.equipmentMaintenance.serviceConsumables).map(([resource, count]) => `${count} ${resource}`).join(" + ") || "no consumables"} · qualification ${Object.entries(result.metrics.equipmentMaintenance.qualificationConsumables).map(([resource, count]) => `${count} ${resource}`).join(" + ") || "no consumables"} · ${result.metrics.equipmentMaintenance.totalDriftedJobs} drifted jobs / ${result.metrics.equipmentMaintenance.totalDriftedLots} lots / ${result.metrics.equipmentMaintenance.totalDriftDefects} defects${Object.entries(result.metrics.equipmentMaintenance.devices).length ? ` · ${Object.entries(result.metrics.equipmentMaintenance.devices).map(([device, maintenance]) => `${device}=${maintenance.jobsSinceMaintenance} jobs/${(maintenance.qualificationAgeTicks / 1000).toFixed(1)} s age/${(maintenance.readyWorkOverlapTicks / 1000).toFixed(1)} s ready overlap${maintenance.qualificationPending ? `/awaiting ${maintenance.qualificationPending.cause}` : ""}`).join(", ")}` : ""}`,
...
export async function designCommand(projectDir: string, programId: string | undefined, options: { run: boolean; runId?: string; continue: boolean; promote?: string; maxCandidates?: number; progress?: string; json: boolean; section?: string; signal?: AbortSignal }): Promise<void> {
  ...
  if (!options.run) {
    ...
    const action = recommendedDesignProgramEvidenceAction(evidence);
    const candidateId = action.runId ? `${programId}-${action.runId.slice(0, 8)}` : null;
    const actionDetail = action.kind === "run"
      ? {
        id: `design.run:${programId}`,
        description: `Create current evidence by evaluating up to ${brief.program.budget.maxCandidates} bounded proposals through locked Benchmark '${brief.benchmark.id}'.`,
        argv: ["inm", "design", brief.project.rootDir, "--program", programId, "--run", "--max-candidates", String(brief.program.budget.maxCandidates), "--json"],
        effect: "creates-artifact" as const,
      }
      : action.kind === "continue"
        ? {
          id: `design.continue:${action.runId}`,
          description: `Continue the exact current frontier with up to ${brief.program.budget.maxCandidates} additional Candidate evaluations.`,
          argv: ["inm", "design", brief.project.rootDir, "--program", programId, "--run-id", action.runId, "--continue", "--max-candidates", String(brief.program.budget.maxCandidates), "--json"],
          effect: "creates-artifact" as const,
        }
        : action.kind === "promote"
          ? {
            id: `design.promote:${action.runId}`,
            description: "Create a hash-pinned Candidate from the exact current promotable Design authority.",
            argv: ["inm", "design", brief.project.rootDir, "--program", programId, "--run-id", action.runId, "--promote", candidateId!, "--json"],
            effect: "creates-artifact" as const,
          }
          : {
    ...
    const data = sectionResult("design", options, {
      summary: () => ({ program: brief.program, benchmark: brief.benchmark, seed: brief.seed, promotionBase: brief.promotionBase, driver: brief.driver, staticEvidence: brief.staticEvidence, evidence: evidenceSummary }),
      static: () => brief.staticEvidence,
      iterations: () => [],
      ...
      best: () => null,
      runs: () => ({ evidence, runs, invalidRuns }),
      all: () => ({ ...brief, evidence, runs, invalidRuns }),
    });
    if (options.json) writeSuccess("design", data, {
      context,
      nextActions: [nextAction(actionDetail.id, actionDetail.description, actionDetail.argv, actionDetail.effect)],
    });
    else write([
      ...
      "",
      `Next: ${actionDetail.argv.slice(0, -1).join(" ")}`, "",
    ].join("\n"), false);
    return;
  }
  const progressMode = (options.progress ?? (options.json ? "off" : "human")) as ProgressMode;
