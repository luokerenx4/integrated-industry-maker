...
export async function planCommand(projectDir: string, selection: ProjectSelection, options: OutputOptions): Promise<void> {
  ...
  write([
    ...
    "", "Raw resources",
    ...plan.rawResources.map((resource) => `  ${resource.resource.padEnd(18)} need ${resource.totalDemandPerMinute.toFixed(3).padStart(8)}/min  extraction ${resource.configuredExtractionPerMinute.toFixed(3).padStart(8)}/min  scheduled ${resource.scheduledSupplyPerMinute.toFixed(3).padStart(8)}/min (${resource.scheduledSupply} lots)  Scenario balance ${resource.scenarioBalance.toFixed(3)}`),
    "", "Transport envelopes",
