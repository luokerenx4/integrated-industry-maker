...
export async function simulateCommand(projectDir: string, selection: ProjectSelection, options: { seed: number; untilTick?: number; maxEvents?: number; json: boolean }): Promise<void> {
  ...
  if (options.json) write(summary, true);
    ...
    `Equipment maintenance: ${result.metrics.equipmentMaintenance.totalCompleted} completed (${result.metrics.equipmentMaintenance.totalMandatory} mandatory / ${result.metrics.equipmentMaintenance.totalOpportunistic} opportunistic) · ${result.metrics.equipmentMaintenance.totalCancelled} cancelled · ${(result.metrics.equipmentMaintenance.totalMaintenanceTicks / 1000).toFixed(3)} s work · ${result.metrics.equipmentMaintenance.totalDriftedJobs} drifted jobs / ${result.metrics.equipmentMaintenance.totalDriftedLots} lots / ${result.metrics.equipmentMaintenance.totalDriftDefects} defects${Object.entries(result.metrics.equipmentMaintenance.devices).length ? ` · ${Object.entries(result.metrics.equipmentMaintenance.devices).map(([device, maintenance]) => `${device}=${maintenance.jobsSinceMaintenance} jobs`).join(", ")}` : ""}`,
