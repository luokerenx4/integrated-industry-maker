  ...
  if (options.json) {
    ...
    ...(result.metrics.lotFlow.family ? [
      ...
      ...Object.entries(result.metrics.cadenceControl.devices).map(([device, control]) =>
        `Cadence control ${device}: ${control.process} · ${control.normalJobs} ${control.normalMode} / ${control.recoveryJobs} ${control.recoveryMode} jobs · recover below ${control.recoverBelowItems} items on ${control.downstreamConnection}`),
    ] : []),
