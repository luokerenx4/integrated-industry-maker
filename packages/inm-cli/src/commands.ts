...
function cadenceControlSide(
  ...
  return `${control.normalJobs} ${control.normalMode} / ${control.recoveryJobs} ${control.recoveryMode} jobs · ${control.recoveryActivations} recovery activations · ${control.process} · recover after ${(control.minimumStarvationTicks / 1000).toFixed(1)}s below ${control.recoverBelowItems} items on ${control.downstreamConnection} · ${control.starvationEpisodes} starvation episodes / ${(control.starvationTicks / 1000).toFixed(1)}s`;
}
  ...
  if (options.json) {
    ...
    ...(result.metrics.lotFlow.family ? [
      ...
      ...Object.entries(result.metrics.cadenceControl.devices).map(([device, control]) =>
        `Cadence control ${device}: ${control.process} · ${control.normalJobs} ${control.normalMode} / ${control.recoveryJobs} ${control.recoveryMode} jobs · ${control.recoveryActivations} recovery activations · recover after ${(control.minimumStarvationTicks / 1000).toFixed(1)}s below ${control.recoverBelowItems} items on ${control.downstreamConnection} · ${control.starvationEpisodes} starvation episodes / ${(control.starvationTicks / 1000).toFixed(1)}s`),
    ] : []),
