...
export async function simulateCommand(projectDir: string, selection: ProjectSelection, options: { seed: number; untilTick?: number; maxEvents?: number; json: boolean }): Promise<void> {
  ...
  if (options.json) write(summary, true);
    ...
    `Equipment maintenance: ${result.metrics.equipmentMaintenance.totalCompleted} completed (${result.metrics.equipmentMaintenance.totalMandatory} mandatory / ${result.metrics.equipmentMaintenance.totalOpportunistic} opportunistic) · ${result.metrics.equipmentMaintenance.totalCancelled} cancelled · ${(result.metrics.equipmentMaintenance.totalMaintenanceTicks / 1000).toFixed(3)} s equipment work / ${(result.metrics.equipmentMaintenance.totalServiceCrewTicks / 1000).toFixed(3)} crew-s · ${(result.metrics.equipmentMaintenance.totalInputWaitTicks / 1000).toFixed(3)} s consumable wait / ${(result.metrics.equipmentMaintenance.totalCrewWaitTicks / 1000).toFixed(3)} s crew wait · ${Object.entries(result.metrics.equipmentMaintenance.serviceConsumables).map(([resource, count]) => `${count} ${resource}`).join(" + ") || "no consumables"} · ${result.metrics.equipmentMaintenance.totalDriftedJobs} drifted jobs / ${result.metrics.equipmentMaintenance.totalDriftedLots} lots / ${result.metrics.equipmentMaintenance.totalDriftDefects} defects${Object.entries(result.metrics.equipmentMaintenance.devices).length ? ` · ${Object.entries(result.metrics.equipmentMaintenance.devices).map(([device, maintenance]) => `${device}=${maintenance.jobsSinceMaintenance} jobs`).join(", ")}` : ""}`,
