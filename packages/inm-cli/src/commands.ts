...
export async function simulateCommand(projectDir: string, selection: ProjectSelection, options: { seed: number; untilTick?: number; maxEvents?: number; json: boolean }): Promise<void> {
  ...
  if (options.json) write(summary, true);
    ...
    ...(result.metrics.lotFlow.family ? [
      ...
      `Release control: ${result.metrics.releaseFlow.control}${result.metrics.releaseFlow.maximumWip === null ? "" : ` max ${result.metrics.releaseFlow.maximumWip} / reopen ${result.metrics.releaseFlow.reopenAtWip} / ${result.metrics.releaseFlow.dispatch}`} · ${result.metrics.releaseFlow.peakActiveLots} peak active · ${result.metrics.releaseFlow.controlBlockedLots} control-blocked lots / ${(result.metrics.releaseFlow.controlBlockedTicks / 1000).toFixed(3)} lot-s`,
