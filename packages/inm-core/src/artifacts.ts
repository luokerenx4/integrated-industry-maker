...
export async function writeRunArtifact(project: CompiledFactoryProject, result: SimulationResult, options: RunArtifactOptions): Promise<RunSummary> {
  ...
  const report = [
    ...
    `- Release control: ${result.metrics.releaseFlow.control}${result.metrics.releaseFlow.maximumWip === null ? "" : ` · max WIP ${result.metrics.releaseFlow.maximumWip} · reopen at ${result.metrics.releaseFlow.reopenAtWip} · ${result.metrics.releaseFlow.dispatch}`} · peak ${result.metrics.releaseFlow.peakActiveLots} active lots · ${result.metrics.releaseFlow.controlBlockedLots} control-blocked / ${(result.metrics.releaseFlow.controlBlockedTicks / 1000).toFixed(3)} lot-s · ${result.metrics.releaseFlow.capacityBlockedLots} capacity-blocked / ${(result.metrics.releaseFlow.capacityBlockedTicks / 1000).toFixed(3)} lot-s`,
    ...
    `- Equipment setup: ${result.metrics.equipmentSetups.totalChangeovers} changeovers · ${(result.metrics.equipmentSetups.totalSetupTicks / 1000).toFixed(3)} s work`,
