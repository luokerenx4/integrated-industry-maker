...
export function findBlueprintConnectionPath(
  ...
  const end = externalPortCell(to, toAsset, connection.to.port);
  ...
  for (const device of blueprint.devices.filter((item) => item.region === from.region)) {
    ...
    const footprint = rotatedFootprint(asset, device.rotation);
    for (let y = device.position.y; y < device.position.y + footprint.height; y++) for (let x = device.position.x; x < device.position.x + footprint.width; x++) blocked.add(`${x},${y}`);
  }
  for (const node of world.resourceNodes.filter((item) => item.region === from.region)) blocked.add(`${node.position.x},${node.position.y}`);
  blocked.delete(`${start.x},${start.y}`); blocked.delete(`${end.x},${end.y}`);
