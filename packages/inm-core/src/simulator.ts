...
export function runUntil(project: CompiledFactoryProject, initialState = createInitialFactoryState(project), options: RunOptions = {}): SimulationResult {
  ...
  const contractCommitmentSourcesByRegion = Object.fromEntries(Object.entries(deliveryContractResourcesByRegion)
    .map(([region, resources]) => {
      const selected = new Set(resources);
      const bufferSourcesByResource = Object.fromEntries(resources.map((resource) => [
        resource,
        devicesByRegion[region]!.flatMap((device) => bufferIdsByDevice[device.id]!
          .filter((buffer) => {
            const accepts = device.buffers[buffer]!.accepts;
            return accepts.includes("*") || accepts.includes(resource);
          })
          .map((buffer) => ({ device: device.id, buffer }))),
      ]));
      const activeOutputDevices = devicesByRegion[region]!.filter((device) =>
        bufferIdsByDevice[device.id]!.some((buffer) => {
          const accepts = device.buffers[buffer]!.accepts;
          return accepts.includes("*") || accepts.some((resource) => selected.has(resource));
        })).map((device) => device.id);
      const connections = connectionIds.filter((connectionId) => {
        const connection = project.connections[connectionId]!;
        return connection.toDevice.region === region && connection.resources.some((resource) => selected.has(resource));
      });
      const logisticsNetworks = logisticsNetworkIds.filter((networkId) =>
        project.logisticsNetworks[networkId]!.routes.some((route) =>
          route.toRegion === region && selected.has(route.resource)));
      return [region, { bufferSourcesByResource, activeOutputDevices, connections, logisticsNetworks }];
    }));
  ...
  const devicesByPowerGrid = Object.fromEntries(powerGridIds.map((grid) => [
    grid,
    project.powerGrids[grid]!.members.map((id) => project.devices[id]!),
  ]));
  const generationDevices = devices.filter((device) => device.generationPlan !== undefined);
  const generationDevicesByPowerGrid = Object.fromEntries(powerGridIds.map((grid) => [
    grid,
    generationDevices.filter((device) => device.powerGrid === grid),
  ]));
  ...
  const transportPowerStages = Object.values(project.connections).flatMap((connection) => connection.logisticsStages.flatMap((stage) => (
    stage.stage === "line" ? [] : [{
      ...
      stage: stage.stage,
      phase: stage.stage === "loader" ? "loading" as const : "unloading" as const,
      device: stage.device!.id,
      powerGrid: stage.powerGrid,
      activeDeltaMilliWatts: Math.max(0, stage.asset.power.activeMilliWatts - stage.asset.power.idleMilliWatts),
    }]
  )));
  const transportPowerStagesByPowerGrid = Object.fromEntries(powerGridIds.map((grid) => [
    grid,
    transportPowerStages.filter((stage) => stage.powerGrid === grid),
  ]));
  ...
  type PriorityPowerConsumer = {
    ...
    kind: "transport";
    device: CompiledDevice;
    connection: CompiledFactoryProject["connections"][string];
    stage: "loader" | "unloader";
    phase: "loading" | "unloading";
  };
  const canOwnActiveJob = (device: CompiledDevice): boolean =>
    !device.transportEndpoint
    && !device.assetDef.capabilities.includes("station")
    && !device.assetDef.maintenanceProvider
    && !device.assetDef.utilityProvider;
  const priorityPowerConsumersByGrid = Object.fromEntries(powerGridIds.map((grid) => [
    grid,
    powerRankedDevicesByGrid[grid]!.flatMap((device): PriorityPowerConsumer[] => {
      const consumers: PriorityPowerConsumer[] = [];
      if (canOwnActiveJob(device)) consumers.push({ kind: "job", device });
      if (device.stationEnergyPlan) consumers.push({ kind: "station-charge", device });
      if (device.transportEndpoint) {
        const stage = device.transportEndpoint.stage;
        consumers.push({
          kind: "transport",
          device,
          connection: project.connections[device.transportEndpoint.connection]!,
          stage,
          phase: stage === "loader" ? "loading" : "unloading",
        });
      }
      return consumers;
    }),
  ...
  const standbyPower = (grid?: string) => {
    let sum = 0;
    for (const device of devicesForPowerGrid(grid)) {
      const runtime = state.devices[device.id]!;
      if (runtime.idlePowered && runtime.status !== "failed") sum += standbyRequirement(device);
    }
    return sum;
  };
  const requestedStandbyPower = (grid?: string) => {
    let sum = 0;
    for (const device of grid === undefined ? connectedDevices : devicesForPowerGrid(grid)) {
      const runtime = state.devices[device.id]!;
      if (runtime.status !== "failed") sum += standbyRequirement(device);
    }
    return sum;
  };
  const activeTransportPowerDelta = (grid?: string) => {
    let sum = 0;
    for (const stage of transportPowerStagesForPowerGrid(grid)) {
      if (!state.transports[stage.connection]!.some((transit) => transit.phase === stage.phase)
        || (!proportionalPower && state.devices[stage.device]!.status !== "processing")) continue;
      sum += stage.activeDeltaMilliWatts;
    }
    return sum;
  };
  const requestedTransportPowerDelta = (grid?: string) => {
    let sum = 0;
    for (const stage of transportPowerStagesForPowerGrid(grid)) {
      if (state.devices[stage.device]!.status === "failed") continue;
      const requested = state.transports[stage.connection]!.some((transit) => transit.phase === stage.phase)
        || transportPowerBlocked[`${stage.connection}:${stage.stage}`];
      if (requested) sum += stage.activeDeltaMilliWatts;
    }
    return sum;
  };
  ...
  const activePower = (grid?: string) => {
    let sum = standbyPower(grid);
    for (const device of devicesForPowerGrid(grid)) {
      const runtime = state.devices[device.id]!;
      if (device.stationEnergyPlan) {
        sum += stationChargePower(device);
        continue;
      }
      if (!runtime.idlePowered || runtime.status !== "processing" || !runtime.activeJob || runtime.activeJob.generationMilliWatts) continue;
      sum += Math.max(0, runtime.activeJob.powerMilliWatts - standbyRequirement(device));
    }
    return sum + activeTransportPowerDelta(grid);
  };
  const requestedPower = (grid?: string) => {
    let sum = requestedStandbyPower(grid);
    for (const device of devicesForPowerGrid(grid)) {
      const runtime = state.devices[device.id]!;
      if (runtime.status === "failed" || runtime.activeJob?.generationMilliWatts) continue;
      if (device.stationEnergyPlan) {
        sum += stationChargeRequestedDelta(device);
        continue;
      }
      const baseline = standbyRequirement(device);
      const requested = runtime.activeJob?.powerMilliWatts
        ?? (!device.transportEndpoint && runtime.status === "unpowered" ? unmetPowerDemand[device.id] ?? baseline : baseline);
      sum += Math.max(0, requested - baseline);
    }
  ...
  const generationPower = (grid?: string) => {
    ...
    for (const device of generationDevicesForPowerGrid(grid)) {
      const runtime = state.devices[device.id]!;
      if (runtime.status === "failed") continue;
      sum += device.generationPlan!.kind === "renewable"
  ...
  const storageDischargePower = (grid?: string) => {
    let sum = 0;
    for (const device of storageDevicesForPowerGrid(grid)) {
      const runtime = state.devices[device.id]!;
      if (runtime.status !== "failed" && (runtime.energyStorage?.storedMilliJoules ?? 0) > 1e-9) {
        sum += device.storagePlan!.dischargeMilliWatts;
      }
    }
    return sum;
  };
  ...
  const syncPowerAvailability = () => {
    for (const grid of powerGridIds) state.energy.grids[grid]!.availableMilliWatts = availablePower(grid);
  ...
  const captureContractCommitments = (resources: readonly string[], region: string): ContractCommitments => {
    const commitments: Record<string, number> = {};
    const sources = contractCommitmentSourcesByRegion[region]!;
    for (const resource of resources) {
      let committed = stats.consumedByRegion[region]?.[resource] ?? 0;
      for (const source of sources.bufferSourcesByResource[resource] ?? []) {
        committed += state.devices[source.device]!.buffers[source.buffer]![resource] ?? 0;
      }
      commitments[resource] = committed;
    }
    for (const deviceId of sources.activeOutputDevices) {
      for (const output of state.devices[deviceId]!.activeJob?.produce ?? []) {
        if (commitments[output.resource] !== undefined) commitments[output.resource]! += output.count;
      }
    }
    for (const connection of sources.connections) for (const transit of state.transports[connection]!) {
      if (commitments[transit.resource] !== undefined) commitments[transit.resource]! += transit.count;
    }
    for (const network of sources.logisticsNetworks) for (const transit of state.logisticsTransports[network]!) {
      if (project.devices[transit.to]?.region === region && commitments[transit.resource] !== undefined) {
        commitments[transit.resource]! += transit.count;
      }
    }
    return commitments;
  };
  const contractCommitted = (resource: string, region: string): number =>
    captureContractCommitments([resource], region)[resource] ?? 0;
  const committedFrom = (commitments: ContractCommitments | undefined, resource: string, region: string): number =>
    commitments?.[resource] ?? contractCommitted(resource, region);
  const contractContribution = (
    device: CompiledDevice,
    plan: CompiledDevice["processPlans"][number],
    executions = 1,
    commitments?: ContractCommitments,
  ): number =>
  ...
  const captureDeviceContractCommitments = (device: CompiledDevice): ContractCommitments | undefined =>
  ...
  const selectBatchFormationProcessPlan = (device: CompiledDevice, commitments?: ContractCommitments): {
  ...
  const selectCampaignProcessPlan = (device: CompiledDevice, commitments?: ContractCommitments): {
    plan?: CompiledDevice["processPlans"][number];
    held: boolean;
    changed: boolean;
  } => {
  ...
  const rebalanceActivePower = (): boolean => {
    ...
    for (const grid of powerGridIds) {
      ...
      for (const device of stationEnergyDevicesByPowerGrid[grid]!) {
        const runtime = state.devices[device.id]!;
        if (stationChargeRequestedDelta(device) <= 0 && runtime.stationEnergy!.chargeSatisfactionPpm !== 0) {
          mutateFactoryState(state, { kind: "station.charge-satisfaction", device: device.id, satisfactionPpm: 0 });
          changed = true;
        }
      }
      for (const consumer of priorityPowerConsumersByGrid[grid]!) {
        const { device, kind } = consumer;
        const runtime = state.devices[device.id]!;
        const job = runtime.activeJob;
        if (kind === "job" && (!job || runtime.status === "failed" || job.generationMilliWatts)) continue;
        if (kind === "station-charge" && stationChargeRequestedDelta(device) <= 0) continue;
        if (kind === "transport" && (runtime.status === "failed"
          || !state.transports[consumer.connection.id]!.some((transit) => transit.phase === consumer.phase))) continue;
        ...
        if (kind === "transport") {
          for (const transit of state.transports[consumer.connection.id]!.filter((item) => item.phase === consumer.phase)) {
  ...
  const settle = () => {
    ...
    while (changed && guard++ < 100_000) {
      ...
      for (const device of generationEvaluationOrder) if (tryEvaluate(device)) generationChanged = true;
      syncPowerAvailability();
