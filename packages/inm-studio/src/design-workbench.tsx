...
function progressLabel(progress: DesignRunProgress): { title: string; detail: string } {
  ...
  if (progress.phase === "objective-target-completed") return {
    ...
    detail: `${progress.objectiveTargetEvidence.target.component}.${progress.objectiveTargetEvidence.target.metric}${progress.objectiveTargetEvidence.target.metric === "averageInventory" ? ` @ ${progress.objectiveTargetEvidence.target.location}` : ""} · ${progress.objectiveTargetEvidence.before} → ${progress.objectiveTargetEvidence.after} · Δ ${signed(progress.objectiveTargetEvidence.delta)}`,
  };
