...
import type { BlueprintBenchmarkProgress, CandidateChangeSet, DesignDecisionEvidence, DesignProgramBrief, DesignProgramSummary, DesignRunProgress, DesignRunResult, DesignRunSummary, InspectedDesignProgramEvidence, InvalidDesignRunSummary, ResearchPromotionBoundary, WorkbenchDesignProgramEvidence } from "@inm/core";
      ...
      : evidence.basis === "current-best-case-guardrail"
        ...
        : evidence.basis === "addressed-loss-not-improved"
          ...
          : evidence.basis === "addressed-objective-not-improved"
            ? "DOES NOT IMPROVE ADDRESSED OBJECTIVE"
...
function focusDetail(program: DesignProgramSummary): { title: string; detail: string } {
  ...
  return {
    ...
    detail: focus.locations?.length
      ? `${focus.locations.length} exact physical exposure location${focus.locations.length === 1 ? "" : "s"}`
      : "exact Objective component",
  };
}
...
function progressLabel(progress: DesignRunProgress): { title: string; detail: string } {
  ...
  if (progress.phase === "proposal-completed") return {
    ...
    detail: `${progress.branch.nodeId} → ${progress.strategy}${progress.addressedCase ? ` · repairs ${progress.addressedCase}` : progress.addressedLoss ? ` · addresses ${progress.addressedLoss}` : progress.addressedObjectiveTarget ? ` · addresses Objective ${progress.addressedObjectiveTarget.component}` : ""}`,
  };
  ...
  if (progress.phase === "objective-target-completed") return {
    title: `OBJECTIVE TARGET ${progress.objectiveTargetEvidence.improved ? "IMPROVED" : "NOT IMPROVED"}`,
    detail: `${progress.objectiveTargetEvidence.target.component}.${progress.objectiveTargetEvidence.target.metric}${progress.objectiveTargetEvidence.target.metric === "averageWipEquivalentUnits" ? ` @ ${progress.objectiveTargetEvidence.target.location}` : ""} · ${progress.objectiveTargetEvidence.before} → ${progress.objectiveTargetEvidence.after} · Δ ${signed(progress.objectiveTargetEvidence.delta)}`,
  };
...
export function DesignWorkbench({
  ...
  const [evidence, setEvidence] = useState<WorkbenchDesignProgramEvidence | null>(null);
  ...
  const loadProgram = async (programId: string) => {
    const value = await responseJson<InspectedDesignProgramEvidence>(await fetch(
      `/api/projects/${encodeURIComponent(projectId)}/designs/${encodeURIComponent(programId)}`,
    ));
    const projectedById = new Map(value.evidence.runs.map((run) => [run.id, run]));
    ...
    if (!selectedRunId && value.evidence.authorityRunId) onSelectRun(value.evidence.authorityRunId);
  };

  useEffect(() => {
    pollAbort.current?.abort();
    setRunning(false); setActiveOperation(null); setBrief(null); setEvidence(null); setRuns([]); setInvalidRuns([]); setSelectedRun(null); setSelectedRunIssue(null); setPromoted(null); setCommissionedCandidate(null); setRunProgress(null); setLastCompletedCase(null); setError(null);
    ...
    <section className="design-workbench" role="dialog" aria-modal="true" aria-label="Factory design workbench" data-testid="design-workbench">
      ...
      <aside className="design-program-list" aria-label="Project Design Programs">
        ...
        {programs.map((program) => <button key={program.id} className={program.id === selectedProgramId ? "selected" : ""} data-testid={`design-program-${program.id}`} onClick={() => onSelectProgram(program.id)}>
          <strong>{program.name}</strong><code>{program.id}</code><span>{focusDetail(program).title} · {program.budget.maxCandidates} MAX · {program.frontier.maximumAlternativeBranches} ALT · {program.locked ? "LOCKED" : "UNLOCKED"}</span>
        </button>)}
      ...
      <div className="design-body">
        ...
        {selectedProgram && brief && <>
          ...
          {selectedRun && selectedRunEvidence?.currentness.state === "historical" && <section className="design-run-issue" data-testid="historical-design-run" role="status">
            <div>
              <small>VALID HISTORICAL EVIDENCE</small>
              <strong>THIS RUN CANNOT OWN THE CURRENT FACTORY DECISION</strong>
              <code>{selectedRun.manifest.resultHash}</code>
              <span>{selectedRunEvidence.currentness.reasons.join(" · ")}</span>
              <p>The immutable result remains readable, but current continuation, promotion, default selection, and loss disposition require exact current Program, Benchmark, seed, driver, promotion-base, and engine identity.</p>
            </div>
            <button data-testid="open-current-design-authority" onClick={() => onSelectRun(evidence?.authorityRunId ?? null)}>
              {evidence?.authorityRunId ? "OPEN CURRENT AUTHORITY →" : "BACK TO PROGRAM →"}
            </button>
          </section>}
          {selectedRun && <section className="design-result" data-testid="design-result">
            ...
            <div className="design-iterations"><div className="design-iteration-head"><span>#</span><span>DECISION</span><span>CAUSAL TARGET → FAMILY / STRATEGY</span><span>SCORE EFFECT</span></div>{selectedRun.manifest.iterations.map((iteration) => <div key={iteration.iteration}><b>{iteration.iteration}</b><i className={iteration.decision.toLowerCase()}>{iteration.decision}</i><div className="design-iteration-evidence"><strong data-testid={iteration.addressedCase ? "design-repair-target" : iteration.addressedObjectiveTarget ? "design-objective-target" : undefined}>{iteration.addressedCase ? `REPAIRS ${iteration.addressedCase}` : iteration.addressedLoss ? `ADDRESSES ${iteration.addressedLoss}` : iteration.addressedObjectiveTarget ? `ADDRESSES OBJECTIVE ${iteration.addressedObjectiveTarget.component}` : "NO EXPLICIT TARGET"} · {iteration.decisionFamily}</strong><code>{iteration.strategy}</code><small>BEFORE {promotionBoundaryDetail(iteration.promotionBoundary)}</small><small>FROM {iteration.frontierEvidence.parent.role.toUpperCase()} {iteration.frontierEvidence.parent.nodeId} → {iteration.frontierEvidence.outcome.toUpperCase()}{iteration.frontierEvidence.pruned.length ? ` · PRUNED ${iteration.frontierEvidence.pruned.map((item) => item.nodeId).join(", ")}` : ""}</small><small>OBSERVED {iteration.driverEvidence.fabLoss?.chain.join(" → ") ?? "no tracked fab loss"}</small>{iteration.lossTargetEvidence && <small data-testid="design-loss-target">CAUSAL TARGET {iteration.lossTargetEvidence.target.contributor}.{iteration.lossTargetEvidence.target.metric} · {iteration.lossTargetEvidence.before} → {iteration.lossTargetEvidence.after} · Δ {signed(iteration.lossTargetEvidence.delta)} · {iteration.lossTargetEvidence.improved ? "IMPROVED" : "NOT IMPROVED"}</small>}{iteration.objectiveTargetEvidence && <small data-testid="design-objective-target-evidence">OBJECTIVE TARGET {iteration.objectiveTargetEvidence.target.component}.{iteration.objectiveTargetEvidence.target.metric}{iteration.objectiveTargetEvidence.target.metric === "averageWipEquivalentUnits" ? ` @ ${iteration.objectiveTargetEvidence.target.location}` : ""} · {iteration.objectiveTargetEvidence.before} → {iteration.objectiveTargetEvidence.after} · Δ {signed(iteration.objectiveTargetEvidence.delta)} · {iteration.objectiveTargetEvidence.improved ? "IMPROVED" : "NOT IMPROVED"}</small>}{iteration.decisionEvidence && <><small>{decisionDetail(iteration.decisionEvidence)}</small><ScoreBreakdownDetails
