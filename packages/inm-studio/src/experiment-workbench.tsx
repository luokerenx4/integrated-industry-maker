        ...
        {selected && <>
          ...
          {activeOperation && <section className={`experiment-live-progress ${activeOperation.status}`} aria-live="polite" data-testid="experiment-progress">
            <div>
              ...
              <span>{progress ? progress.phase.endsWith("completed") ? `${progress.cached ? "REUSED" : "EVALUATED"}${progress.execution.mode === "parallel" ? ` · PARALLEL ×${progress.execution.concurrency}` : ""}${progress.timing.workerReused === undefined ? "" : progress.timing.workerReused ? " · WARM WORKER" : ` · COLD WORKER · ${(progress.timing.workerStartupMs ?? 0).toFixed(0)}ms STARTUP`} · ${((progress.timing.durationMs ?? 0) / 1000).toFixed(2)}s` : `RUNNING EXACT LOCKED CASE${progress.execution.mode === "parallel" ? ` · PARALLEL ×${progress.execution.concurrency}` : ""}` : activeOperation.error?.message ?? "LOADING PROJECT-LOCAL CASES"}</span>
            </div>
