...
interface IndustrialAnalysis {
  ...
  connections: Array<{
    ...
    resources: string[];
    dispatchPolicy: "fifo" | "round-robin" | "shortage-first" | "batch-coherent";
  ...
  return <section className="scene-inspector" aria-label={`Device inspector: ${device.id}`} data-testid={`device-inspector-${device.id}`}>
    ...
    <div className="scene-inspector-scroll">
      ...
      <div className="inspector-facts">
        ...
        {device.cadenceControl && <span><small>CADENCE CONTROL</small><b>{device.cadenceControl.normalMode.toUpperCase()} → {device.cadenceControl.recoveryMode.toUpperCase()} {device.cadenceControl.kind === "downstream-coverage-recovery"
          ? `AFTER ${(device.cadenceControl.minimumCoverageDeficitTicks / 1000).toFixed(1)}S COVERAGE DEFICIT BELOW ${device.cadenceControl.recoverBelowItems} · ${device.cadenceControl.downstreamConnection}`
          : `AFTER OLDEST ${device.cadenceControl.inputResource} LOT WAITS ${(device.cadenceControl.minimumQueueTicks / 1000).toFixed(1)}S WITH ${device.cadenceControl.recoverAtItems}+ RESIDENT ITEMS`}</b></span>}
        ...
        {cadenceControl?.kind === "downstream-coverage-recovery" && <span><small>COVERAGE DEFICIT OBSERVED</small><b>{cadenceControl.coverageDeficitEpisodes} EPISODES / {(cadenceControl.coverageDeficitTicks / 1000).toFixed(1)}S</b></span>}
        ...
        <section className="analysis-section logistics-analysis">
          ...
          <div className="pipeline-list">{analysis.connections.map((connection) => {
            ...
            return <div className="pipeline-card" key={connection.connection}>
              ...
              <footer><span>{connection.dispatchPolicy.toUpperCase()}{connection.dispatchPolicy === "shortage-first" || connection.dispatchPolicy === "batch-coherent" ? ` · ${connection.dispatchProfiles.map((profile) => `${profile.resource}${profile.minimumTreatmentLevel ? `@${profile.minimumTreatmentLevel}+` : ""}:${profile.targetKind}/B${profile.coverageUnit}/D${profile.criticalDepth ?? "-"}`).join(" + ")}` : ""}</span><span>{flow ? `MEASURED ${(flow.utilization * 100).toFixed(1)}% · ${flow.blockedItemTicks} BLOCKED ITEM-TICKS` : `DISPATCH ${connection.dispatchIntervalTicks}ms`}</span><span>LATENCY {connection.travelTicks}ms</span><span>PATH {connection.pathCells} CELLS{connection.maxLevel ? ` · LEVEL ${connection.maxLevel}` : ""}{connection.sharedCells ? ` · ${connection.sharedCells} SHARED` : ""}</span></footer>
            </div>;
          })}</div>
        </section>
...
function RunsOverview({
  ...
  const sourceInvestigation = new URLSearchParams(window.location.search)
    .get("investigation")?.trim() || null;
                    ...
                    ([device, control]) => (
                      <Fragment key={device}>
                        ...
                        <Metric
                          ...
                          value={control.kind === "downstream-coverage-recovery"
                            ? `< ${control.recoverBelowItems} items · ${control.downstreamConnection}`
                            : `${control.recoverAtItems}+ ${control.inputResource} · oldest ${(control.minimumQueueTicks / 1000).toFixed(1)} s`}
                        />
                      </Fragment>
                    ),
                  )}
