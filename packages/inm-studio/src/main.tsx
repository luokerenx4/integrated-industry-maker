...
interface Metrics {
  ...
  equipmentMaintenance: {
    ...
    totalInputWaitTicks: number; totalCrewWaitTicks: number; totalInputBlocks: number; totalCrewBlocks: number; totalServiceCrewTicks: number;
    ...
    devices: Record<string, {
      ...
      serviceConsumables: Record<string, number>; wait?: { reason: "consumable" | "crew"; sinceTick: number };
    }>;
    providers: Record<string, {
      crews: number; crewsInUse: number; peakCrewsInUse: number; assignments: number; completed: number; cancelled: number;
      serviceCrewTicks: number; consumables: Record<string, number>;
    }>;
  };
  ...
  return <section className="scene-inspector" aria-label={`Device inspector: ${device.id}`}>
    ...
    <div className="scene-inspector-scroll">
      ...
      <div className="inspector-facts">
        ...
        {device.maintenance && <span><small>QUALIFIED PROVIDERS</small><b>{device.maintenanceProviders.map((provider) => `${provider.device} @ ${provider.distance.toFixed(1)}`).join(" · ") || "UNCOVERED"}</b></span>}
        ...
        {maintenance && <span><small>SERVICE WAIT</small><b>{formatTick(maintenance.inputWaitTicks)} INPUT / {formatTick(maintenance.crewWaitTicks)} CREW · {maintenance.inputBlocks + maintenance.crewBlocks} BLOCKS</b></span>}
        ...
        <article className="asset-detail">
          {selected && <>
            ...
            {selected.type === "device" ? <>
              ...
              {selected.production?.maintenance && <section className="asset-section"><h4>Usage drift & maintenance</h4><div className="asset-table"><div><b>MANDATORY AFTER {selected.production.maintenance.maximumJobs} JOBS</b><strong>{formatTick(selected.production.maintenance.durationTicks)} maintenance</strong><span>resets usage state</span><code>{(selected.production.maintenance.powerMilliWatts / 1000).toFixed(1)} W</code></div>{(selected.production.maintenance.drift ?? []).map((stage) => <div key={stage.afterJobs}><b>AFTER {stage.afterJobs} JOBS</b><strong>{stage.durationMultiplier.numerator}/{stage.durationMultiplier.denominator}× time · {stage.powerMultiplier.numerator}/{stage.powerMultiplier.denominator}× power</strong><span>{stage.defects.length ? stage.defects.join(" · ") : "no defect injection"}</span><code>active until maintenance</code></div>)}</div></section>}
              {selected.production?.maintenance && <section className="asset-section"><h4>Maintenance service contract</h4><div className="asset-table"><div><b>{selected.production.maintenance.service.crews}× {selected.production.maintenance.service.skill}</b><strong>{selected.production.maintenance.service.inputs.map((input) => `${input.count} ${input.resource}`).join(" + ") || "no consumables"}</strong><span>allocated for complete physical service</span><code>shared provider capacity</code></div></div></section>}
