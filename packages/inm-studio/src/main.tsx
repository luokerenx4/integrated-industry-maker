...
interface Metrics {
  ...
  equipmentMaintenance: {
    ...
    devices: Record<string, { jobsSinceMaintenance: number; completed: number; mandatory: number; opportunistic: number; cancelled: number; maintenanceTicks: number; driftedJobs: number; driftedLots: number; driftDefects: number }>;
  };
  ...
  return <main className={loading ? "syncing" : ""}>
    ...
    <section className="workspace">
      ...
      <aside>
        ...
        {data.metrics && Object.keys(data.metrics.equipmentMaintenance.devices).length > 0 && <div className="panel"><h2>Equipment maintenance</h2><div className="metrics"><Metric label="MANDATORY / EARLY" value={`${data.metrics.equipmentMaintenance.totalMandatory} / ${data.metrics.equipmentMaintenance.totalOpportunistic}`} /><Metric label="COMPLETED / CANCELLED" value={`${data.metrics.equipmentMaintenance.totalCompleted} / ${data.metrics.equipmentMaintenance.totalCancelled}`} /><Metric label="MAINTENANCE WORK" value={`${(data.metrics.equipmentMaintenance.totalMaintenanceTicks / 1000).toFixed(1)} s`} /><Metric label="DRIFTED JOBS / LOTS" value={`${data.metrics.equipmentMaintenance.totalDriftedJobs} / ${data.metrics.equipmentMaintenance.totalDriftedLots}`} /><Metric label="DRIFT DEFECTS" value={String(data.metrics.equipmentMaintenance.totalDriftDefects)} /></div></div>}
