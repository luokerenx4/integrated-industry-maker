...
interface Metrics {
  ...
  productionUtilities: {
    ...
    devices: Record<string, { allocations: number; completed: number; cancelled: number; occupiedTicks: number; unitTicks: number; inputWaitTicks: number; inputBlocks: number; utilities: Record<string, { allocations: number; unitsAllocated: number; unitTicks: number }> }>;
    providers: Record<string, { capacity: Record<string, number>; reserved: Record<string, number>; peakReserved: Record<string, number>; allocations: number; completed: number; cancelled: number; occupiedTicks: number; unitTicks: number; utilities: Record<string, { allocations: number; unitsAllocated: number; unitTicks: number }> }>;
  };
  equipmentSetups: {
  ...
  return <section className="scene-inspector" aria-label={`Device inspector: ${device.id}`}>
    ...
    <div className="scene-inspector-scroll">
      ...
      <div className="inspector-facts">
        ...
        {utilities && <span><small>UTILITY ALLOCATIONS</small><b>{utilities.allocations} · {utilities.completed} COMPLETE / {utilities.cancelled} CANCEL</b></span>}
  ...
  return <main className={loading ? "syncing" : ""}>
    ...
    <section className="workspace">
      ...
      <aside>
        ...
        {data.metrics && data.metrics.productionUtilities.totalAllocations > 0 && <div className="panel"><h2>Fab facility utilities</h2><div className="metrics"><Metric label="JOBS / COMPLETE" value={`${data.metrics.productionUtilities.totalAllocations} / ${data.metrics.productionUtilities.totalCompleted}`} /><Metric label="CANCELLED" value={String(data.metrics.productionUtilities.totalCancelled)} /><Metric label="JOB / CAPACITY TIME" value={`${(data.metrics.productionUtilities.totalOccupiedTicks / 1000).toFixed(1)} / ${(data.metrics.productionUtilities.totalUnitTicks / 1000).toFixed(1)} s`} /><Metric label="WAIT / BLOCKS" value={`${(data.metrics.productionUtilities.totalInputWaitTicks / 1000).toFixed(1)} s / ${data.metrics.productionUtilities.totalInputBlocks}`} /><Metric label="UTILITY SERVICES" value={Object.entries(data.metrics.productionUtilities.utilities).map(([utility, measured]) => `${utility}: ${measured.unitsAllocated} units / ${(measured.unitTicks / 1000).toFixed(1)} unit-s`).join(" · ") || "NONE"} /></div></div>}
