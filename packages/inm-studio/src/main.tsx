...
interface Metrics {
  ...
  equipmentMaintenance: {
    totalCompleted: number; totalMandatory: number; totalOpportunistic: number; totalUsageTriggered: number; totalCalendarTriggered: number; totalCancelled: number; totalMaintenanceTicks: number;
    ...
    devices: Record<string, {
      jobsSinceMaintenance: number; qualifiedAtTick: number; qualificationAgeTicks: number; usageTriggered: number; calendarTriggered: number; completed: number; mandatory: number; opportunistic: number; cancelled: number; maintenanceTicks: number;
      ...
      qualificationPending?: { cause: "mandatory" | "opportunistic"; trigger: "usage" | "calendar"; jobsSinceMaintenance: number; qualificationAgeTicks: number };
  ...
  return <section className="scene-inspector" aria-label={`Device inspector: ${device.id}`} data-testid={`device-inspector-${device.id}`}>
    ...
    <div className="scene-inspector-scroll">
      ...
      <div className="inspector-facts">
        ...
        {device.preventiveMaintenance && <span><small>PREVENTIVE WINDOW</small><b>{device.preventiveMaintenance.minimumJobs !== undefined ? `AFTER ${device.preventiveMaintenance.minimumJobs} JOBS` : ""}{device.preventiveMaintenance.minimumJobs !== undefined && device.preventiveMaintenance.minimumQualificationTicks !== undefined ? " · " : ""}{device.preventiveMaintenance.minimumQualificationTicks !== undefined ? `AFTER ${formatTick(device.preventiveMaintenance.minimumQualificationTicks)} AGE` : ""}</b></span>}
