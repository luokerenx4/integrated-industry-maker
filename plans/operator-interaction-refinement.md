...
- [ ] The Overview names exactly one recommended next action, explains why it is recommended from shared snapshot facts, and reaches the exact diagnostic, Candidate, run, or operation in one deliberate click.
- [ ] Readiness, effective selection, highest-priority issues, delivery contracts, latest immutable evidence, and review queue remain visible without giving every available Core operation equal first-screen weight.
- [ ] Advanced operations are progressively disclosed, retain effect/scope/guard/CLI detail, and copy controls visibly acknowledge success.
- [ ] Header navigation uses stable destinations, and close, Escape, browser back, reload, and Factory selection clearing preserve route/UI agreement.
- [ ] At 390 px and desktop widths, the project identity and all primary destinations remain usable with no horizontal page overflow or one-column context sprawl.
- [ ] Studio tests, type checking, documentation checks, and actual browser QA pass without mutating checked-in project files.
...
- [ ] Update design documentation and complete HTTP/browser/full-suite verification.
- [ ] Final completion audit, commit, and push.
...
- Actual desktop and 390 px browser QA is pending. Browser control rejected localhost page access under its URL policy, so source inspection and HTTP evidence are not being treated as visual proof.
